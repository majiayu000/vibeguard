...
| L6 | Clear small work executes directly; plan only major architecture or explicit requests |
...
- Clear bounded tasks execute directly.
- Plan only major architecture, migrations, cross-system policy work, or explicit user requests.
- Normal specs are at most two files and about 300 lines total; docs-only work, small bugs, and mechanical edits are exempt.
- Use one writable session per repository and do not delegate by default.
- Stop review at zero findings and pass; initiate at most two review rounds per PR.
- Do not create validators for spec validators.
