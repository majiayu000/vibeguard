...
assert_cmd "enable-codex-hooks preserves prefixed feature keys" grep -Eq '^hooks_beta[[:space:]]*=[[:space:]]*false$' "${CONFIG_FILE}"
...
assert_cmd "enable-codex-hooks adds exact key under commented features table" grep -Eq '^hooks[[:space:]]*=[[:space:]]*true$' "${CONFIG_FILE}"
