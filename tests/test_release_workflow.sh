...
RUNTIME_INSTALL_SH="${REPO_DIR}/scripts/setup/runtime-install.sh"
...
assert_contains "$workflow_text" $'  no-clone-smoke:\n    name: No-clone install smoke (${{ matrix.os }})\n    needs: stage-release\n    runs-on: ${{ matrix.os }}\n    timeout-minutes: 20\n    permissions:\n      # GitHub requires contents: write for workflow tokens to read draft releases.\n      contents: write\n      attestations: read' "no-clone smoke can read the staged draft release"
...
assert_contains "$toolchain_text" 'components = ["clippy", "llvm-tools-preview", "rustfmt"]' "repo rust-toolchain installs CI lint and coverage components"
...
assert_contains "$ci_text" 'CARGO_LLVM_COV_VERSION: "0.8.7"' "CI pins cargo-llvm-cov version"
...
assert_contains "$ci_text" "rustup target add x86_64-unknown-linux-musl aarch64-unknown-linux-musl" "Linux CI installs both release musl targets"
assert_contains "$ci_text" "cargo check --locked --target x86_64-unknown-linux-musl --manifest-path vibeguard-runtime/Cargo.toml" "Linux CI checks x86_64 musl release compatibility"
assert_contains "$ci_text" "cargo check --locked --target aarch64-unknown-linux-musl --manifest-path vibeguard-runtime/Cargo.toml" "Linux CI checks aarch64 musl release compatibility"
assert_contains "$ci_text" "Validate U-22 coverage gate contract" "CI validates the U-22 coverage gate deterministically"
assert_contains "$ci_text" "Enforce measured Rust line coverage baseline" "CI runs the measured U-22 coverage gate"
assert_contains "$ci_text" "bash scripts/ci/self-application/check-u22-coverage.sh" "CI invokes the canonical U-22 coverage script"
...
assert_contains "$workflow_text" "sha256sum vibeguard-runtime-* vibeguard-payload-*.tar.gz | sort -k2,2 > SHA256SUMS" "checksums cover runtime and payload assets in deterministic sorted layout"
assert_not_contains "$workflow_text" "sha256sum vibeguard-runtime-* | sort" "checksums cannot omit payload assets"
assert_not_contains "$workflow_text" "sha256sum vibeguard-payload-*.tar.gz | sort" "checksums cannot omit runtime assets"
