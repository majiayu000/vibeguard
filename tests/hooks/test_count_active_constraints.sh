...
if [[ -f "${CLAUDE_COMPACT_SRC}" && -f "${CODEX_COMPACT_SRC}" ]]; then
  ...
  assert_contains "${claude_core_json}" '"total": 17' "Python counter reports the exact Claude payload budget"
  ...
  assert_exit_zero "Claude payload retains every compact constraint ID" test "${claude_compact_ids}" = "U-29,W-02,W-03,W-12,W-16,SEC-01,SEC-02,SEC-11,SEC-13"
  claude_runtime_json="$("${RUNTIME_BIN}" active-constraints --root "${CLAUDE_CORE_REPO}" --home "${CLAUDE_CORE_HOME}" --host claude --json)"
  assert_contains "${claude_runtime_json}" '"total": 17' "production counter matches the exact Claude payload count"
  ...
  assert_contains "${codex_core_json}" '"total": 18' "Python counter reports the exact Codex payload budget"
  ...
  assert_exit_zero "Codex payload retains every compact constraint ID" test "${codex_compact_ids}" = "U-29,W-02,W-03,W-12,W-16,SEC-01,SEC-02,SEC-11,SEC-13"
  codex_runtime_json="$("${RUNTIME_BIN}" active-constraints --root "${CODEX_CORE_REPO}" --home "${CODEX_CORE_HOME}" --host codex --json)"
  assert_contains "${codex_runtime_json}" '"total": 18' "production counter matches the exact Codex payload count"
