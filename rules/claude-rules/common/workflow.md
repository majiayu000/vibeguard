...
**Eval 假通过模式（来源：[ConfidentAI, 2026-04](https://www.confident-ai.com/blog/three-ways-ai-systems-fail-even-when-evals-pass)）**：

Eval 全部通过不等于系统正确。以下三种模式会导致 eval 绿灯但生产环境失败：

| 模式 | 描述 | 检测方法 |
|------|------|----------|
| 捷径学习（Shortcut Learning） | 工具/函数选对了，但执行路径错误——模型学到了表面模式而非真实推理链。例如 RAG 系统选对了检索 API 但用错了 filter 参数，eval 只检查"是否调用了检索"就判通过。 | 不只断言最终结果，必须断言**中间步骤的正确性**（参数值、调用顺序、中间状态）。对 agent 类系统，验证 tool call trace 而非仅验证最终输出。 |
| 步骤跳过（Step Skipping） | 模型学到某些步骤（如输入验证、权限检查、数据清洗）在 eval 中不影响结果，于是跳过。生产环境中这些步骤的缺失导致安全漏洞或数据损坏。 | 为关键步骤设置**独立的必经断言**（assertion on intermediate checkpoint），不要只靠端到端结果推断步骤是否执行。使用 trace/log 验证步骤实际执行。 |
| 高置信错答（Confidence Miscalibration） | 模型对错误答案给出高置信度，eval 框架基于置信度阈值放行。根因是 eval 数据集与生产分布偏移，模型的置信度在训练分布内校准但在 OOD 场景失效。 | 在 eval 集中**主动注入对抗样本和边界 case**，验证模型在不确定时是否降低置信度或拒绝回答。禁止仅凭置信度 > 阈值就判定正确。 |

**机械化检查（Eval 假通过防御）**：
- 编写 eval/测试时，禁止仅断言最终输出——至少包含一个中间步骤断言（防捷径学习）
- Agent 类系统的测试必须验证 tool call sequence，不只验证最终 response（防步骤跳过）
- eval 数据集每次更新时，必须包含 >= 10% 的对抗/边界样本（防置信度失校准）
- 发现 eval 通过率 > 95% 但用户报告问题时，优先怀疑 eval 覆盖盲区而非"用户操作问题"
