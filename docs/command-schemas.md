...
`scripts/skill_validate.py` appends this artifact as JSONL under `${VIBEGUARD_HOME:-${HOME}/.vibeguard}/artifacts/skill-validate/` unless `--no-persist` or an explicit `--output-dir` is used.
