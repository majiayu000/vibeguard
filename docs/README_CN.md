...
bash ~/vibeguard/setup.sh --profile full  # full：额外启用 Stop Gate + Post-Build-Check + Pre-Commit Hook
```
...
VibeGuard 通过两条路径注入规则：

**路径 1：原生规则（`~/.claude/rules/vibeguard/`）** — 88 条规则通过 Claude Code 原生 rules 机制加载，支持 `paths` 作用域（语言特定规则仅在匹配文件时激活），直接影响 AI 推理层。

**路径 2：CLAUDE.md 注入（`~/.claude/CLAUDE.md`）** — 七层约束索引追加到用户级全局配置。Claude Code 启动时加载所有层级的 CLAUDE.md 并叠加生效：

```
企业级  /Library/Application Support/ClaudeCode/CLAUDE.md   ← IT 部署
用户级  ~/.claude/CLAUDE.md                                  ← VibeGuard 规则索引在这里
...
| AI 要执行 `git push --force`、`rm -rf`、`reset --hard` | `pre-bash-guard` | **拦截** — 给出安全替代方案（子命令感知，不误拦 `git add -f`） |
...
### 原生规则（`rules/claude-rules/` → `~/.claude/rules/vibeguard/`）

88 条规则通过 Claude Code 原生 rules 机制加载，在 AI 推理层生效（不仅是脚本拦截）：

| 目录 | 内容 | 作用域 |
|------|------|--------|
| `common/coding-style.md` | U-01 ~ U-26 通用约束（含 U-25 构建失败修复优先、U-26 声明-执行完整性） | 全局 |
| `common/data-consistency.md` | U-11 ~ U-14 跨入口数据一致性 | 全局 |
| `common/security.md` | SEC-01 ~ SEC-10 安全规则 | 全局 |
| `common/workflow.md` | W-01 ~ W-05 工作流约束（调试协议、连续失败后退、验证后声称） | 全局 |
| `rust/quality.md` | Rust 专项 | `**/*.rs, **/Cargo.toml` |
| `golang/quality.md` | Go 专项 | `**/*.go, **/go.mod` |
| `typescript/quality.md` | TypeScript 专项 | `**/*.ts, **/*.tsx` |
| `python/quality.md` | Python 专项 | `**/*.py` |

> **注意**：Claude Code 用户级 `~/.claude/rules/` 的 YAML `paths` 数组解析有 bug（[#21858](https://github.com/anthropics/claude-code/issues/21858)），必须用 CSV 单行格式（如 `paths: "**/*.rs,**/Cargo.toml"`）。

### 行内排除

守卫脚本支持 `// vibeguard:ignore` 行内注释跳过单行检测：

```go
result := dangerousOp() // vibeguard:ignore
```

目前支持：RS-03（unwrap）、GO-01（error handling）、GO-02（goroutine leak）、TS-01（any abuse）。
...
│   ├── run-hook.sh                       #   Hook 执行入口
│   ├── pre-write-guard.sh                #   新文件拦截
│   ├── pre-bash-guard.sh                 #   危险命令拦截（子命令感知）
│   ├── pre-edit-guard.sh                 #   防幻觉编辑
│   ├── pre-commit-guard.sh               #   git commit 前自动守卫（10s 超时）
...
│   ├── rust/                             #   Rust 守卫（含 Taste Invariants、声明-执行鸿沟检测）
