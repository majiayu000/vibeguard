...
### RS-14: 声明-执行鸿沟检测（已禁用）
- **场景**: 4 个子检测全有严重误报
  1. struct field 计数用 `rg -A 50` + grep 行数，注释/变量绑定全误匹配
  2. trait 检测对外部 crate trait 和 marker trait 误报
  3. save/load 只查 main.rs/lib.rs，不查模块间调用
  4. 未接入 common.sh，`cd` 改变 cwd 影响后续守卫
- **影响**: 任何正常 Rust 项目必触发
- **修复**: 暂时禁用（`exit 0`），待用 AST 工具（rust-analyzer/syn）重写
- **文件**: `guards/rust/check_declaration_execution_gap.sh`
...
### TS-01: any 检测误命中注释和字符串
- **场景**: 块注释 `/* type: any */` 和字符串 `"schema: any"` 内的 `: any` 被误报
- **影响**: 含注释或字符串描述的 TS 文件误报
- **修复**: 追加过滤：排除块注释开头行（`/\*`）和赋值字符串行（`= "..."`）
- **文件**: `guards/typescript/check_any_abuse.sh`

### GO-01: range 变量误报
- **场景**: `for _, v := range slice` 中的 `_` 被当作 error 丢弃
- **影响**: 所有用 range 的 Go 文件
- **修复**: guard 和 post-edit hook 均排除 `for.*range` 和 `ok/found/exists` 模式
- **文件**: `guards/go/check_error_handling.sh`, `hooks/post-edit-guard.sh`
