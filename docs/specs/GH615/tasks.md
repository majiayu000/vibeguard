...
- [ ] `SP615-T2` Owner: `/root` — 在 `vibeguard-runtime/src/hook_orchestrator/mod.rs` 实现 same-session Grep/Glob boundary 之后的 visible reminder counter，保留 attempt telemetry、breaker flow、500-event bound 与 mode/config semantics，并改正 escalation copy。Depends on: SP615-T1。Covers: B-001-B-008。Done when: focused red tests转绿；silent attempts 不计数；有效搜索恢复；无效事件不恢复；重新忽略 reminders 会再次阻断。Verify: focused shell test 与 Rust tests。
...
- `lane_map`: specification `/root` 独占 `docs/specs/GH615/`、triage artifact 与 spec index；
  implementation `/root` 独占 `vibeguard-runtime/src/hook_orchestrator/mod.rs`、
