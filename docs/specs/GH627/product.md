...
2. B-002 namespaced 输入必须匹配闭集 manifest contract；未知名称、空 canonical
   名、缺失/空 requested name、额外前缀、目录分隔符或 traversal 片段必须 fail closed，
   且绝不能执行目标 hook。`PreToolUse` 必须输出 protocol-valid `permissionDecision: deny`，
   `PermissionRequest` 必须输出 `behavior: deny`，`Stop` 必须输出 `stopReason`，其他已知事件
   必须输出可见失败；这些输出保留稳定的 invalid-hook-name 原因后以 wrapper exit 0 返回，
   不能用空输出或仅 diagnostic + exit 0 表示拒绝。
...
5. B-005 删除 alias 文件后，安装模块、manifest/目录同步检查、`packs/safe-bash/pack.yaml`
   与 focused Codex/guard-pack tests 必须证明没有残留物理依赖；safe-bash 的 Codex 配置仍以
   requested `vibeguard-pre-bash-guard.sh` 调用 wrapper，但 source/install 文件列表只能分发
   canonical `hooks/pre-bash-guard.sh`。
...
- [ ] 非法/缺失名称负例输出事件级可见拒绝、稳定原因、wrapper exit 0，且零目标执行、零路径穿越。
- [ ] dev-linked/installed snapshot、manifest、safe-bash guard pack 与 adapter regression tests 通过。
