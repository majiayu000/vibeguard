...
| Manifest validator | `scripts/lib/hooks_manifest.py:269` | 要求 `codex.script` 指向物理 namespaced 文件 | 需要校验 requested name 与同 entry canonical `script` 的精确映射，而不是 alias 文件存在性 |
...
`hooks_manifest.py` 保留 `codex.script` 必须以 `vibeguard-` 开头的外部命名约束，但把物理
namespaced 文件存在检查替换为精确关系 `codex.script == "vibeguard-${item.script}"`，并继续由
entry 的 canonical `item.script` 文件存在检查保证可执行目标有效。manifest contract 负例必须
覆盖错配 requested/canonical 名称，防止 wrapper allowlist 与 manifest 漂移。
...
| B-005 | file deletion + manifest validator/set-sync + safe-bash pack contract | alias glob 为空；manifest requested name 精确等于 `vibeguard-${canonical}` 且只要求 canonical 文件存在；pack source/install 无 alias，Codex audit 命令仍传 requested name；`bash tests/test_manifest_contract.sh` 与 `bash tests/test_guard_packs.sh` 通过 |
...
- [ ] Integration: `bash tests/test_codex_runtime.sh`、installed snapshot、`bash tests/test_manifest_contract.sh` 与 `bash tests/test_guard_packs.sh`。
