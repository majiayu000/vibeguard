...
`{authority_id,backend,authority_identity_digest,policy_epoch,policy_bundle_digest,client_api,control_api,control_approval_policy,publication_store_path,publication_store_lock_path,volume_identity,authority_kms,retention_policy_digest,trust_bundle_digest,blocked_attempt_ledger,trusted_time_service,bootstrap_governance,break_glass_governance,restore_anchor_service,restore_backup_service,anchor_signing_policy,predicate_evaluator_roster}`。
该 persisted history deployment manifest只携 deployment policy；dedicated
`prebootstrap_policy_binding`只存在于 `prepare_bootstrap_trusted_time` control request/response branch，禁止写入、
迁入或别名为 deployment policy。authority API投影另须 byte-equal machine schema的 exact KMS ARN、authenticated
DescribeKey 64hex KeyMaterialId attestation与 manifest key-binding digest。
