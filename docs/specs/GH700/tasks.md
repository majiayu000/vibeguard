...
- [ ] `SP700-T10` 接入 strict summary、serialized evidence 与 two-phase publication：summary 绑定全部 required/display-only reports且 digest排除自身；attempts/reconciler/publish 共用 candidate/source-identity、repository-global ledger 与 renewable/fenced publication leases。phase A 创建 private draft并重验 assets；valid phase B 在 repository publication lease 下以 default-branch CAS 合并去旧 marker PR、写绑定 lease token 的 receipt，再 attest intent/publish，并继续持 lease/CAS 到 rollback 或 new-current merge；publish_nonvalid 跳过 marker mutation。failure manifest 绑定完整 inputs/global ledger identity；pre-attestation tuple含 API-verified ref且只绑定 exact-ref candidate。Covers: B-016, B-017, B-018, B-021, B-022, B-027, B-029, B-031. Owner: release/summary/reconciler owner. Depends on: SP700-T2, SP700-T3, SP700-T5, SP700-T7–T9。Done when: two valid candidates overlap、stale/expired fence、summary/missing-input、partial draft、distinct-ref same-commit/unbound record、repository-wide blocked frontier、cancel before/after marker/intent/commit、zero-marker rollback、nonvalid preserve-current矩阵收敛到最多一个 current marker；无 unsigned/public partial assets。Verify: `bash tests/test_public_benchmark.sh`; `bash tests/test_release_workflow.sh`.
...
- [ ] `SP700-T12` 建立完整 adversarial harness，新增 mapping identity-field injection、ambient PATH/fake-or-missing executable、real/fake runtime grant、branch-delayed undeclared exec、broker bypass/loss/race、baseline `nearest_rank_p95_v1` boundary、distinct-ref same-commit/unbound binding、跨 source blocked-ledger rewrite、两 valid candidate 并发 marker transition、expired fence/CAS、receipt/rollback window、publish_nonvalid preserve-current，以及既有 launcher/limits/privacy/cancel/digest/reviewer/two-phase矩阵。Covers: B-001–B-031. Owner: integration-test owner. Depends on: SP700-T2–SP700-T11。Done when:每个 mutation断言具体 artifact/state/exit，最多一个 current marker、无 public partial release，用户 canaries不变且所有非valid路径非零。Verify: `bash tests/test_public_benchmark.sh`; `bash tests/test_behavior_eval.sh`; `bash tests/test_hook_perf_contract.sh`; `bash tests/test_release_workflow.sh`.

- [ ] `SP700-T13` 在同一 immutable implementation head 完成生产回归、安全审查与 current-head merge evidence：运行全部 Rust、benchmark、behavior、hook、release、docs 和 broad ordinary contract checks；independent reviewer 核对 identity、logical-ID-only mapping、runtime grant/per-exec broker、closed baseline estimator、privacy/cleanup、denominators/digests、repository-global ledger frontier、exact-ref binding、repository publication lease/CAS、marker-before-intent/rollback 与 required-platform gate，SEC-11 reviewer另审 command execution。Covers: B-001–B-031. Owner: verification owner + independent read-only reviewer + human security reviewer. Depends on: SP700-T12；GH-699 T3–T6 merged/no-clone launcher evidence；current-head CI/review/thread/merge state。Done when: fresh checks 全绿、findings 为零，head/required CI/0 threads/reviews/merge state一致；merge/release仍按授权。Verify: 运行本文件“验证”中的普通仓库命令并记录证据。
