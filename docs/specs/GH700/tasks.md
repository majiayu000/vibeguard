...
- [ ] `SP700-T3` 作为唯一 publication-authority store/deployment、blocked-attempt ledger、trusted-time/high-water与 restore-anchor implementation single writer，原样实现 [publication_history_contract.md](publication_history_contract.md) 指定的 production backends、双认证 API、schema/fold、deploy/bootstrap/migrate/recover/restore、trust/governance、mutation/broker/recovery与 conformance vectors；不得在 task或调用方复制 machine-facing字段/枚举/closed union或提供 local/mock fallback。实现路径固定为 `vibeguard-runtime/src/publication_authority/{mod.rs,store.rs,broker.rs,recovery.rs,restore_anchor.rs,blocked_attempt_ledger.rs,trusted_time.rs}`+main CLI；authority、ledger、trusted-time与 anchor的 planned schema/workflow/bootstrap路径均按 contract实现。T3独占 contract定义的 manifest-pinned transport/trust、durable persistence inventory、broker credential、trusted-time/high-water、conditional-CAS proof与 restore execution；独立 maintainer governance只持 bootstrap/rotation/restore threshold approval与 anchor admin，T10只可调认证 client/ledger API。Covers: B-004, B-016, B-019, B-022, B-025–B-027, B-029, B-031. Owner: schema/integrity + publication-authority/ledger/time/restore-anchor implementation owner. Depends on: T1。Done when:contract-owned vectors覆盖 complete history/attempt unions、永久 retention/recovery、bootstrap/first-frontier、no-draft/deleted-draft proof、RFC3161 quorum/high-water rollback、无环 rotation、双 API identity/policy drift、blocked precedence、takeover/governance、secret/capsule、closed inventory及 anchor table replacement/concurrency/stale CAS/ack loss/DB↔anchor crash/threshold restore；真实 durable-volume、独立 TSA signers与隔离 DynamoDB test table证明无丢失/重复/rollback/unsigned frontier，mock不算通过。Verify: `python3 scripts/ci/validate_public_benchmark.py`; `cargo test --manifest-path vibeguard-runtime/Cargo.toml publication_authority`; `bash tests/test_public_benchmark.sh`.
