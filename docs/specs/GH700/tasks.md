...
- [ ] `SP700-T10` 接入 strict summary、serialized evidence 与 two-phase publication：summary digest排除自身且 detached attest；summary 绑定最终展示的全部 required/display-only reports，但 validity 只消费 required subset；attempts/reconciler/publish共用 candidate/source-identity leases、repository-global ledger lease 与 watermark。phase A 创建 private attempt draft并重验 assets；valid phase B 必须先合并去旧 marker 的 human-reviewed PR、freshness 验证并永久写 `marker_transition_receipt`，再 attest 绑定 receipt 的 publish_intent，以 draft→published 为唯一 commit point；transition 后 intent 前取消必须经 human-reviewed rollback PR 恢复旧 marker，publish_nonvalid 跳过 marker mutation。target 与 release-aggregation failure manifest 分别绑定单 target或 present/missing完整 failed-summary inputs及 global ledger identity；pre-attestation server tuple 必须含 API-verified ref，early record 只绑定 exact-ref candidate，绝不由同 commit 的下一个 candidate 接管。Covers: B-016, B-017, B-018, B-021, B-022, B-027, B-029, B-031. Owner: release/summary/reconciler owner. Depends on: SP700-T2, SP700-T3, SP700-T5, SP700-T7–T9。Done when: summary/missing-input、partial draft、distinct-ref same-commit early identity、unbound record、repository-wide concurrent blocked frontier、cancel before/after marker/intent/commit、valid zero-marker rollback、nonvalid preserve-current/unmarked-row、permission/watermark/duplicate delivery矩阵各自收敛到唯一合法状态；无未签名展示附件或 public partial assets。Verify: `bash tests/test_public_benchmark.sh`; `bash tests/test_release_workflow.sh`.
