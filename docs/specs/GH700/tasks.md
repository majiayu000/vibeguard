...
- [ ] `SP700-T10` 接入 strict summary、serialized evidence 与统一 publication state machine：所有 actors 使用 server-authenticated `repo_node_id`，严格按 source/candidate→repository ledger→repository publication→branch CAS 取短 lease；在任何 marker/Release/README 可见动作前写 `publication_ownership_record(prepared)`。valid 闭集区分 ledger/CAS 证明的 `genesis_zero` 与单 current 的 `rollover_one`，intent 绑定 receipt + pre-approved new-current patch；publish_nonvalid intent 绑定 reviewed unmarked-row patch。prepared owner 必须覆盖 commit/状态写间 crash、pre-intent rollback、post-commit marker与nonvalid row；PR拒绝/关闭/超时/owner消失统一由 higher-fence reconciler 创建同 candidate exact reviewed replacement，无 replacement则对应 `*_recovery_blocked` 并阻断下一 publication，exact rollback/delete或README merge后才 terminal。Covers: B-016, B-017, B-018, B-021, B-022, B-027, B-029, B-031. Owner: release/summary/reconciler owner. Depends on: SP700-T2, SP700-T3, SP700-T5, SP700-T7–T9。Done when: genesis/no-op禁止、owner crash at every state edge、rollback/new-current/nonvalid-row rejected/closed/stale/deadline、two candidates、reverse-lock deadlock、stale fence/CAS、repo-name/node-ID drift矩阵均无无主状态/双 current/漏 row。Verify: `bash tests/test_public_benchmark.sh`; `bash tests/test_release_workflow.sh`.
