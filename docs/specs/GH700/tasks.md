...
- [ ] `SP700-T3` 在明确的 **schemas/public_benchmark_protocol.schema.json** 建立 protocol ownership，并建立 corpus/truth/mapping/report/summary/failure-manifest/review-record/publication-history schemas、JCS canonicalizer、reviewer verifier与 corpus-ledger validator。publication frontier 为 `(repo_node_id,length,root,full_prefix_digest)`；history transition分为 fence-independent immutable intent、mutable append authorization envelope与 store-signed committed envelope/receipt。op ID绑定 repo/owner-generation/run/slot/predecessor/kind/payload而不含 authorization fence；request另携 current fence/lease/actor。store先查事务 repo+op unique index：same digest返原 receipt，异 digest冲突；absent才验 current generation/fence/predecessor并 append/sign。ack-loss：committed op返原 receipt；uncommitted old fence失败，同 generation续租后用 same intent+new fence重授权；advanced禁止 rebase并按 suffix恢复或 new predecessor/generation/slot重规划；takeover前未提交旧 generation永久失效。versioned JCS union覆盖所有 record与六类 blocked，fold拒绝 ABA。Covers: B-004, B-016, B-019, B-022, B-025, B-026, B-027, B-029, B-031. Owner: schema/integrity owner. Depends on: T1。Done when:每种 record 的 commit前 timeout/commit后 ack-loss/restart+renewed-fence、uncommitted old-fence fail/new-fence same-op once、committed old/new-fence same receipt、concurrent fences、same-ID same/different digest、takeover-before/after append、terminal/replan suffix、generation/fence wrap/reuse、index/receipt/envelope tamper、truncate/fork矩阵全绿。Verify: `python3 scripts/ci/validate_public_benchmark.py`; `cargo test --manifest-path vibeguard-runtime/Cargo.toml bench::digest`; `bash tests/test_public_benchmark.sh`.
...
- [ ] `SP700-T12` 建立完整 adversarial harness，覆盖每种 history record 的 commit前 timeout、commit后 ack-loss、restart+renewed fence、uncommitted old-fence fail/new-fence same-op once、committed old/new-fence same receipt、old/new并发、same-ID same/different digest、takeover-before/after append、0/1/N terminal/replan/incompatible suffix、owner-generation/fence wrap/reuse、index/receipt/envelope tamper、fork/truncate；draft覆盖 API loss/stale-zero/分页/权限/nonce/exhaustive-negative/upload；四 PR kind覆盖 ref/push/create/bind loss、all-state/multipage/spoof/wrong creator/fork、迟到 PR、cancel/late bind/merge、replacement/multilevel/reopen/ref ABA/stale gate；以及 post-intent/permissions/六 blocked。Covers: B-001–B-031. Owner: integration-test owner. Depends on: T2–T11。Done when:所有 mutation/append 断言 unique fold/fence authorization/receipt/outcome，最多一个 merge-authorized PR/current marker，无 stale append/duplicate transition/orphan/late merge/ownerless Release/漏 row。Verify: `bash tests/test_public_benchmark.sh`; `bash tests/test_behavior_eval.sh`; `bash tests/test_hook_perf_contract.sh`; `bash tests/test_release_workflow.sh`.
