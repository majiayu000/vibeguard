...
- [ ] `SP700-T3` 作为唯一 publication-authority store/deployment single writer，原样实现 [publication_history_contract.md](publication_history_contract.md) 指定的 production backend、认证 API、schema/fold、deploy/bootstrap/recover、trust/governance、mutation/broker/recovery与 conformance vectors；不得在 task或调用方复制 machine-facing字段/枚举/closed union或提供 local/mock fallback。实现路径固定为 `vibeguard-runtime/src/publication_authority/{mod.rs,store.rs,broker.rs,recovery.rs}`+main CLI，部署/初始化路径固定为 planned **schemas/publication_authority_deployment.schema.json**+**.github/workflows/publication-authority-deploy.yml**+**scripts/ci/bootstrap_publication_authority.py**；T3独占 contract定义的 manifest-pinned transport/trust、durable persistence inventory、broker credential、rollback-resistant external restore anchor与 restore authority，T10只可调认证 API。Covers: B-004, B-016, B-019, B-022, B-025–B-027, B-029, B-031. Owner: schema/integrity + publication-authority store/deployment owner. Depends on: T1。Done when:三种 consumer共用 contract-owned vectors并覆盖 no-draft/deleted-draft terminal proof、无环 rotation、API trust、blocked precedence、takeover/governance suffix、secret/capsule反例、closed persistence inventory及 anchor rollback；真实 durable-volume fixture对 transaction-boundary kill/power-loss、并发 writer/send、WAL/fsync/disk/KMS failure、runner/checkout删除及 signed snapshot+WAL restore证明无丢失/重复/unsigned frontier。Verify: `python3 scripts/ci/validate_public_benchmark.py`; `cargo test --manifest-path vibeguard-runtime/Cargo.toml publication_authority`; `bash tests/test_public_benchmark.sh`.
...
- [ ] `SP700-T10` 接入 strict summary与统一 publication client/machine，只消费 T3 manifest-pinned authenticated authority API，不得拥有/替换 backend、直接开 DB、降级 local store或重定义 contract identifiers；client GitHub token只读，target write credential仅 authority/broker持有。每个 Release write、uncertain recovery、effect closure、blocked precedence、takeover、governance suffix与 post-invalidation fold均原样消费 [publication history contract](publication_history_contract.md) 的 machine；authority unavailable或 proof不完整时零 remote mutation。Covers: B-016–B-018, B-021, B-022, B-027, B-029, B-031. Owner: publication client owner. Depends on: T2/T3/T5/T7–T9。Done when:API trust/ambient-discovery/credential-leak反例与所有 contract mutation kinds的 crash/discovery/partial/compensation/takeover均 fail closed，no-draft与deleted-draft terminal分支均可验证。Verify: `bash tests/test_public_benchmark.sh`; `bash tests/test_release_workflow.sh`.
...
- [ ] `SP700-T12` 建立完整 adversarial harness，所有 publication cases直接参数化 contract-owned vectors。post-invalidation正例分别覆盖 no-draft与 deleted-draft evidence，并拒绝 missing/unknown/wrong cleanup tag、missing/wrong deletion evidence、publish intent、public Release及任一 pending/blocked/in-flight slot；另覆盖 governance/takeover及 forged/nonterminal/cross-candidate/current-restoration反例。authority vectors覆盖无环 rotation/cutover、API location/transport/server+client trust、redirect/ambient discovery、credential leak、external anchor rollback、persistence inventory delete/tamper与 blocked precedence。所有 contract mutation kinds覆盖 crash/0/1/N/in-flight/partial/compensation/takeover及 effect closure；capsule覆盖 construction order、替换/泄漏/placeholder/send-once；invalidation与 guard覆盖各自 contract反例。Covers: B-001–B-031. Owner: integration-test owner. Depends on: T2–T11。Done when:所有 slot处于 closed terminal set且 effect closure成立，最多一 draft/public Release/exact assets。Verify: `bash tests/test_public_benchmark.sh`; `bash tests/test_behavior_eval.sh`; `bash tests/test_hook_perf_contract.sh`; `bash tests/test_release_workflow.sh`.
