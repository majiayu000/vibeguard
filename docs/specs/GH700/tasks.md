...
- [ ] `SP700-T3` 作为唯一 publication-authority store/deployment、blocked-attempt ledger、trusted-time/high-water、encrypted restore backup与 restore-anchor implementation single writer，建立 [publication_history_contract.md](publication_history_contract.md) 指定的 `publication_authority_sqlite_v1`、`blocked_attempt_ledger_sqlite_v1`、`rfc3161_tsa_quorum_v1`、`publication_restore_backup_s3_object_lock_v1`、bounded online anchor signer quorum及独立账户 `publication_restore_anchor_dynamodb_v1` contracts、schema/fold、deploy/bootstrap/migrate/recover/restore。实现路径固定为 `vibeguard-runtime/src/publication_authority/{mod.rs,store.rs,broker.rs,recovery.rs,restore_anchor.rs,backup_store.rs,anchor_signer.rs,governance_recovery.rs,blocked_attempt_ledger.rs,trusted_time.rs}`+main CLI，部署/初始化路径固定为 planned **schemas/publication_authority_deployment.schema.json**、**schemas/publication_history.schema.json**、**schemas/blocked_attempt_ledger.schema.json**、**.github/workflows/publication-authority-deploy.yml**、**scripts/ci/bootstrap_publication_authority.py**及 planned **.github/workflows/publication-restore-anchor-deploy.yml**、**.github/workflows/publication-restore-backup-deploy.yml**、**scripts/ci/bootstrap_publication_restore_anchor.py**、**scripts/ci/bootstrap_publication_restore_backup.py**；T3独占同时必需、共享 authority/policy identity 的 HTTPS-mTLS `client_api`与 Unix-peercred `control_api`，durable SQLite/WAL/FULL fsync、38-kind complete union、永久 attempt ledger、RFC3161 high water、bootstrap+break-glass roots、global pending anchor gate、AEAD-encrypted snapshot/manifest/WAL exact Object-Lock versions、routine least-privilege online quorum lifecycle及 DynamoDB identity/CAS/restore execution。独立 maintainer governance只持 privileged bootstrap/rotation/restore/emergency approval与 anchor/backup admin，routine signers不持 maintainer keys，T10只调认证 APIs。history/attempt frontiers exact且各 authorization domain独立；normal rotation无自引用，emergency root cutover须 anchored predecessor/restore proof/audit/new root。Covers: B-004, B-016, B-019, B-022, B-025–B-027, B-029, B-031. Owner: all authority/history/attempt/time/backup/signer/anchor schemas and lifecycle；T3执行，独立 governance批准 privileged actions。Depends on: T1。Done when:共用 goldens逐一覆盖38 kinds、四 attempt kinds、permanent retention、backup encryption/identity/restore、routine signer threshold/lifecycle、pending commit serialization/ack-loss、break-glass expiry/threshold-loss cutover、trusted time、dual API、mutation与 external CAS；真实 durable volume+Object-Lock bucket+独立 signers/TSA+DynamoDB证明无丢失/重复/rollback且 mock不算通过。Verify: `python3 scripts/ci/validate_public_benchmark.py`; `cargo test --manifest-path vibeguard-runtime/Cargo.toml publication_authority`; `bash tests/test_public_benchmark.sh`.
...
- [ ] `SP700-T12` 建立完整 adversarial harness：schema-complete vectors逐一覆盖38 history kinds exact payload与4 attempt kinds，拒绝 unknown/alias/missing/extra/null drift；blocked ledger用真实 durable volume覆盖永久 state。真实隔离 Object-Lock bucket覆盖 snapshot/manifest/WAL AEAD、exact version/resource/KMS/retention、丢失/篡改与 full restore；独立 online signers覆盖 quorum不足、wrong class、credential replay/expiry、key overlap/rotation/revocation且证明不使用 maintainer keys；global pending gate覆盖并发 local successor及 DB commit→backup→signature→anchor CAS→local confirmation全部 crash/ack-loss windows。RFC3161覆盖 high-water races；bootstrap/break-glass覆盖 active leaf/root expiry、current threshold loss、wrong recovery roster/cause/delay/audit/new root、anchor/backup rollback与首次 valid前 emergency record。另覆盖 dual APIs、DynamoDB replacement/stale/single-winner、terminal suffix、六 mutation、capsule/invalidation/guard；mock不计通过，closed inventory逐项删除/tamper fail。Covers: B-001–B-031. Owner: integration-test owner（只写 fixture/test）。Depends on: T2–T11。Done when:所有 history/attempt/slot effect closure成立，最多一 local pending/successor、draft/public Release且 time/frontier/backup anchor均不回退。Verify: `bash tests/test_public_benchmark.sh`; `bash tests/test_behavior_eval.sh`; `bash tests/test_hook_perf_contract.sh`; `bash tests/test_release_workflow.sh`.

- [ ] `SP700-T13` 在同一 immutable head完成回归/evidence；independent reviewer核对 complete unions、blocked-ledger retention、bootstrap/break-glass governance、RFC3161 high-water、normal/emergency rotation、dual API、encrypted Object-Lock backup、online quorum credential lifecycle、pending-gate serialization/ack-loss、external anchor、closed inventory与 mutation recovery；SEC-11审 read-only reconciler、authority/broker-only write credential、routine signer/maintainer-key隔离、TSA/backup/anchor trust、break-glass audit、outbox/capsule/KMS及 command execution。Covers: B-001–B-031. Owner: verification owner + independent reviewer + human security reviewer. Depends on: T12；GH-699 T3–T6；current-head gates。Done when:fresh checks、CI、0 threads/reviews/merge state一致；merge/release仍按授权。Verify: current-head证据。
