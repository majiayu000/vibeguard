...
- [ ] `SP700-T3` 在明确的 **schemas/public_benchmark_protocol.schema.json** 建立 protocol ownership，并建立 corpus/truth/mapping/report/summary/failure-manifest/review-record/publication-history schemas、JCS canonicalizer、reviewer verifier与 corpus-ledger validator；corpus ledger/publication history 分域。publication frontier 为 `(repo_node_id,length,root,full_prefix_digest)`；每个 history transition冻结跨进程 operation ID、exact predecessor/fence、canonical record bytes/digest，store同事务维护 repo+operation unique index并签发绑定 predecessor/successor的 receipt。ack不确定须重放 signed latest prefix：exact op/digest接受，absent+exact predecessor同 bytes重试，advanced禁止 rebase并按合法 suffix恢复或新 fence/op重规划，其它 blocked。versioned union使用 `jcs-rfc8785-v1` 覆盖 owner-claim/draft-binding/prepared/generated-PR plan|binding|revocation/receipt/intent/commit/takeover/六类 blocked/recovered/terminal，deterministic fold拒绝非法 transition/ABA。mapping 只接受 logical IDs；summary preimage 删除 `summary_digest`；pre-attestation interruption使用 server-auth early identity；JSON number限 safe integers。Covers: B-004, B-016, B-019, B-022, B-025, B-026, B-027, B-029, B-031. Owner: schema/integrity owner. Depends on: SP700-T1与genesis approval。Done when: canonical goldens、history每种 record 的 commit前 timeout/commit后 ack-loss/restart retry、exact op+0/1/N suffix、absent exact/advanced predecessor、takeover/terminal/replan/incompatible suffix、same-ID same/different digest、timestamp drift、index/receipt tamper、truncate/fork、两 actor/fence reuse/ABA矩阵全绿。Verify: `python3 scripts/ci/validate_public_benchmark.py`; `cargo test --manifest-path vibeguard-runtime/Cargo.toml bench::digest`; `bash tests/test_public_benchmark.sh`.
...
- [ ] `SP700-T10` 接入 strict summary 与统一 publication state machine：所有 actors 使用 `repo_node_id`，按 source/candidate→ledger→publication→branch CAS；每次 history append遵循 SP700-T3 operation-ID/same-bytes ack-loss算法，禁止盲目重复 transition。首次 Release API/PR mutation前 CAS `owner_claimed`，draft response后/upload前 `draft_bound`，重验后 `prepared`。四类 original/replacement PR在首次 ref/commit/PR mutation前 `generated_pr_planned(kind)`，response后 bound；lost response完整枚举所有 states+ref，active/merged/closed分别 bind/恢复/revoke，ordinary zero保owner且不重发，仅线性化 exhaustive-negative证明 absence，其它按 kind blocked。rollover latest-frontier gate/revoke receipt与 fresh fence/head/nonce/review拒绝 late bind/merge/reopen/ABA。valid/nonvalid 均先 CAS `intent_written`且它是 commit state唯一 predecessor。reconciler仅 `actions: read`/`contents: write`/`pull-requests: write`；pre-intent只 cleanup/revoke/rollback，post-intent matching Release/draft完成并只写 recovered，皆无→release blocked。六类 blocked保留 owner。Covers: B-016, B-017, B-018, B-021, B-022, B-027, B-029, B-031. Owner: publication owner. Depends on: T2/T3/T5/T7–T9。Done when: claim/binding/prepared/四PR plan|bind|revoke/receipt/intent/commit/takeover/blocked/recovered/terminal每个 ack-loss，及 draft/PR API response-loss、all-state/分页/stale-zero、cancel/merge/replacement/ABA与并发矩阵均无 duplicate transition/orphan/late merge/双 current/漏 row。Verify: `bash tests/test_public_benchmark.sh`; `bash tests/test_release_workflow.sh`.
...
- [ ] `SP700-T12` 建立完整 adversarial harness，覆盖 mapping/runtime/baseline/repo/ledger/fence/lock/genesis/history及每种 history record 的 commit前 timeout、commit后 ack-loss、restart exact retry、0/1/N suffix、absent exact/advanced predecessor、takeover/terminal/replan/incompatible suffix、same-ID same/different digest、timestamp drift、unique-index/receipt tamper、fork/truncate、actor race/fence ABA；draft覆盖 API response loss/stale-zero/分页/权限/nonce/exhaustive-negative/upload；四 PR kind覆盖 ref-create/push/PR-create/bound-append response loss、all-state matches、多页/partial/spoof/wrong creator/fork head、迟到 PR、cancel races、late bind/merge、replacement response-loss/并存/多级/reopen/ref ABA/stale gate；以及 post-intent/permissions/六类 blocked。Covers: B-001–B-031. Owner: integration-test owner. Depends on: T2–T11。Done when:所有 mutation/append 断言 unique fold/ownership/fence/receipt/outcome/PR/marker/release/row/exit，最多一个 merge-authorized PR/current marker，无 duplicate transition/orphan/late merge/ownerless Release/漏 row。Verify: `bash tests/test_public_benchmark.sh`; `bash tests/test_behavior_eval.sh`; `bash tests/test_hook_perf_contract.sh`; `bash tests/test_release_workflow.sh`.
