...
- [ ] `SP700-T3` 在明确的 **schemas/public_benchmark_protocol.schema.json** 建立 protocol ownership，并建立 corpus/truth/mapping/report/summary/failure-manifest/review-record schemas、JCS canonicalizer、maintainer-attested reviewer roster verifier和 published+blocked frontier ledger validator；永久 ledger append 必须用 monotonic fence 对 expected sequence/root 原子 compare-and-append，拒绝 expired fence/竞争 successor。mapping schema 只接受 required asset logical IDs 并拒绝 digest/size/version/path identity；JSONL digest framing 固定为 schema-valid records 按 UTF-8 case ID 排序后的单一 JCS array；summary preimage 固定为删除 `summary_digest` 字段后的完整 object，detached attestation不入 preimage；release aggregation 对每个 target 使用 present/missing union，pre-attestation interruption 使用 server-authenticated early identity branch；JSON number 仍限 safe integers。Covers: B-004, B-016, B-019, B-022, B-025, B-026, B-027, B-029, B-031. Owner: schema/integrity owner. Depends on: SP700-T1与genesis approval。Done when: Rust/Python JSONL framing与summary golden一致，raw-line/concatenated-JCS/line-order drift、self-placeholder/attestation-in-preimage拒绝；mapping identity injection、missing target null identity、pre-attestation event/API identity、distinct fake reviewer IDs with same key/subject、blocked-suffix rewrite与expired-fence competing append矩阵全绿；safe integer/ledger mutation矩阵全绿。Verify: `python3 scripts/ci/validate_public_benchmark.py`; `cargo test --manifest-path vibeguard-runtime/Cargo.toml bench::digest`; `bash tests/test_public_benchmark.sh`.
...
- [ ] `SP700-T10` 接入 strict summary、serialized evidence 与统一 publication state machine：所有 actors 使用 server-authenticated `repo_node_id`，严格按 source/candidate→repository ledger→repository publication→branch CAS 取短 lease；在任何 marker/Release/README 可见动作前写入受认证、retained append-only `publication_history` 的 `publication_ownership_record(prepared)`。valid 闭集区分 ledger/CAS 证明的 `genesis_zero` 与单 current 的 `rollover_one`；genesis 只排除 exact current `(repo_node_id,candidate,run,attempt,fence)` prepared owner，任何其它 active owner 均拒绝，并以 retained history 证明无历史 valid publication。rollover merge receipt/zero receipt 均须在 intent 前 durable；intent 再绑定 receipt + pre-approved new-current patch，publish_nonvalid intent 绑定 reviewed unmarked-row patch。prepared owner 必须覆盖 commit/状态写间 crash、pre-intent rollback、post-commit marker与nonvalid row；PR拒绝/关闭/超时/owner消失统一由具 `actions: read`/`contents: write`/`pull-requests: write` least permissions 的 higher-fence reconciler 创建同 candidate exact reviewed replacement。已有 intent/Release 的 cancelled attempt 必须完成 publication并记录 `recovered_publication`，不得同时写 block-release interruption；无 replacement则对应 `*_recovery_blocked` 并阻断下一 publication，exact rollback/delete或README merge后才 terminal且 history 不删除。Covers: B-016, B-017, B-018, B-021, B-022, B-027, B-029, B-031. Owner: release/summary/reconciler owner. Depends on: SP700-T2, SP700-T3, SP700-T5, SP700-T7–T9。Done when: genesis self-owner通过/other-owner拒绝、history distinguishes genesis/removed-marker、pre-intent receipt loss、post-intent cancel outcome exclusivity、permission-denial、owner crash at every state edge、rollback/new-current/nonvalid-row rejected/closed/stale/deadline、two candidates、reverse-lock deadlock、stale fence/CAS、repo-name/node-ID drift矩阵均无无主状态/双 current/漏 row。Verify: `bash tests/test_public_benchmark.sh`; `bash tests/test_release_workflow.sh`.

- [ ] `SP700-T11` 只从 SP700-T10 owner-bound summary 生成 exact README/locale patches：genesis 用 zero-marker receipt且不建 no-op de-current PR；rollover 生成 reviewed de-current/rollback；valid intent 绑定 reviewed new-current，nonvalid intent 绑定 reviewed unmarked row。所有 original/replacement PR 必须同 summary、同 patch intent并重新 human review。Covers: B-008, B-015, B-016, B-017, B-018, B-020, B-023, B-031. Owner: docs generator owner. Depends on: SP700-T9, SP700-T10；README human review gate。Done when: genesis、rollover、rollback/new-current/nonvalid-row merge与 rejected/closed/stalled replacement、三类 recovery-blocked goldens全通过；最多一个 current marker且无 ownerless zero-marker/public Release。Verify: `python3 scripts/ci/render_public_benchmark.py --check`; `bash scripts/ci/validate-doc-paths.sh`; `bash scripts/ci/validate-doc-command-paths.sh`.

- [ ] `SP700-T12` 建立完整 adversarial harness，新增 mapping identity injection、runtime broker、baseline boundary、`repo_node_id` exact-ref、global ledger rewrite、fenced atomic competing append、固定/反向锁序、genesis/removed-marker history与rollover、两 candidate、expired fence/CAS，以及 prepared→receipt→intent→commit 每条 edge crash、post-intent cancellation outcome exclusivity、reconciler permission denial、rollback/new-current/nonvalid-row rejected/closed/stalled、replacement review、三类 recovery block 与后续 denial 矩阵。Covers: B-001–B-031. Owner: integration-test owner. Depends on: SP700-T2–SP700-T11。Done when:每个 mutation断言 ownership/fence/receipt/history/outcome/PR/marker/release/row/exit，最多一个 current marker、无无主 zero-marker/public Release或漏 row，用户 canaries不变。Verify: `bash tests/test_public_benchmark.sh`; `bash tests/test_behavior_eval.sh`; `bash tests/test_hook_perf_contract.sh`; `bash tests/test_release_workflow.sh`.
