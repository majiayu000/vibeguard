...
- [ ] `SP700-T10` 接入 strict summary、serialized evidence 与 two-phase publication：summary digest排除自身且 detached attest；summary 绑定最终展示的全部 required/display-only reports，但 validity 只消费 required subset；attempts/reconciler/publish共用 lease+watermark。phase A 只创建 private attempt draft、上传并重验全部 assets；phase B 先 attest publish_intent，只有 desired state=valid 才合并并重验去除旧 current marker 的 human-reviewed PR，再以 draft→published 为唯一 commit point；publish_nonvalid 跳过 marker mutation。reconciler 通过 protected environment 只获得精确 draft mutation authority；target 与 release-aggregation failure manifest 分别绑定单 target或 present/missing 完整 failed-summary inputs及 candidate ledger identity；staged identity 缺失时以 server tuple 派生 source-lineage/early-attempt keys，在 source lease 路由，后续按双 lease canonical 顺序绑定并 union 到 candidate watermark。Covers: B-016, B-017, B-018, B-021, B-022, B-027, B-029, B-031. Owner: release/summary/reconciler owner. Depends on: SP700-T2, SP700-T3, SP700-T5, SP700-T7–T9。Done when: summary/missing-input、partial draft、pre-attestation key collision/ambiguous binding、cancel before/after intent/commit、blocked-suffix rewrite、valid zero-marker gap、nonvalid preserve-current/unmarked-row、permission/watermark/duplicate delivery矩阵各自收敛到唯一合法状态；无未签名展示附件或 public partial assets。Verify: `bash tests/test_public_benchmark.sh`; `bash tests/test_release_workflow.sh`.

- [ ] `SP700-T11` 只从 SP700-T10 验签 summary 内绑定的 required/display-only reports 生成 README/locale marker：valid publication 前生成只删除旧 current marker 的双 locale PR，commit 后生成添加新 current 的 PR；publish_nonvalid 只添加永不带 current 的同版本空 metric row并保留已有 latest-valid marker；blocked 无 candidate row。每个平台显示 provenance/effectiveness/status/link，每个 latency surface 独立显示。Covers: B-008, B-015, B-016, B-017, B-018, B-020, B-023, B-031. Owner: docs generator owner. Depends on: SP700-T9, SP700-T10；README human review gate。Done when: valid de-current/zero-gap/new-current 与 nonvalid preserve-current/unmarked-row goldens全通过；任一时刻最多一个 current marker，游离附件、手工数字/列漂移被 freshness拒绝。Verify: `python3 scripts/ci/render_public_benchmark.py --check`; `bash scripts/ci/validate-doc-paths.sh`; `bash scripts/ci/validate-doc-command-paths.sh`.

- [ ] `SP700-T12` 建立完整 adversarial harness，新增 single-registry receipt/manifest/mapping identity drift、ambient PATH/fake-or-missing `git`/undeclared child exec、baseline boundary、missing-target null identity、pre-attestation source/attempt key collision与late binding、blocked-ledger-suffix rewrite、valid de-current window、publish_nonvalid preserve-current/unmarked-row，以及既有 launcher/limits/privacy/cancel/digest/reviewer/two-phase adversarial矩阵。Covers: B-001–B-031. Owner: integration-test owner. Depends on: SP700-T2–SP700-T11。Done when:每个 mutation断言具体 artifact/state/exit，public partial release/stale或nonvalid current marker不存在，用户 canaries不变且所有非valid路径非零。Verify: `bash tests/test_public_benchmark.sh`; `bash tests/test_behavior_eval.sh`; `bash tests/test_hook_perf_contract.sh`; `bash tests/test_release_workflow.sh`.
