...
- [ ] `SP700-T3` 建立全部 benchmark/publication schemas与 fold。history authorization分 publication owner domain与 owner-free governance domain；leaf/root rotation/revocation使用 stable rotation ID、独立 governance lease/fence/threshold approval，pre-state验证并从 successor切换，phase-neutral且两域 fence隔离。另定义六种 Release mutation的 planned/bound/recovery/not-applied/compensation records、slot unique index、完整 kind tuple/discovery receipts，以及 invalidation planned（禁 merge SHA/timestamp）/post-merge receipt schemas。Covers: B-004, B-016, B-019, B-022, B-025–B-027, B-029, B-031. Owner: schema/integrity owner. Depends on: T1。Done when:terminal/no-owner及 active-owner rotation、same rotation重规划幂等、cross-domain fence、old/new cutover；每 mutation slot schema/fold；forged pre-merge SHA均 fail closed。Verify: `python3 scripts/ci/validate_public_benchmark.py`; `cargo test --manifest-path vibeguard-runtime/Cargo.toml bench::digest`; `bash tests/test_public_benchmark.sh`.
