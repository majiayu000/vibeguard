...
- [ ] `SP700-T10` 接入 strict summary 与统一 publication state machine：所有 actors 使用 `repo_node_id`，按 source/candidate→ledger→publication→branch CAS；首次 Release API/PR mutation前 CAS `owner_claimed`，draft create response后/upload前 `draft_bound`，重验后 `prepared`。所有 de-current/rollback/new-current/nonvalid-row original/replacement PR 都在首次 ref/commit/PR mutation前写 `generated_pr_planned(kind)`，response后 `generated_pr_bound(kind)`；lost response完整枚举所有 PR states+ref，唯一 active/merged/closed分别 bind/恢复/revoke，ordinary zero保owner且不重发，仅 PR+ref线性化 exhaustive-negative证明 absence，其它按 kind blocked。history 验证签名 current head/full prefix并对完整 frontier+fence CAS；rollover latest-frontier merge gate、pending revoke receipt与 replacement fresh fence/head/nonce/review拒绝 late bind/merge/reopen/ABA。valid/nonvalid 均先 CAS `intent_written`，它是两类 Release commit state 的唯一 predecessor。reconciler least permissions为 `actions: read`/`contents: write`/`pull-requests: write`。pre-intent只 cleanup/revoke/rollback；post-intent matching Release/draft完成并只写 `recovered_publication`，皆无→`release_recovery_blocked`。六类 blocked保留 owner；只有 authenticated negative或 exact cleanup/revocation/rollback/Release+README才 terminal。Covers: B-016, B-017, B-018, B-021, B-022, B-027, B-029, B-031. Owner: release/summary/reconciler owner. Depends on: SP700-T2, SP700-T3, SP700-T5, SP700-T7–T9。Done when: draft及四 PR kind×ref-create/push/PR-create/bind response-loss、all-state/多页/stale-zero/permission/API/nonce/creator/fork-base matches、cancel-vs-create/bind/queue/merge、replacement/reopen/ABA/ruleset、history predecessor、post-intent、六类 blocked与并发矩阵均无 orphan/late merge/双 current/漏 row。Verify: `bash tests/test_public_benchmark.sh`; `bash tests/test_release_workflow.sh`.
...
- [ ] `SP700-T12` 建立完整 adversarial harness，新增 mapping/runtime/baseline/repo identity/ledger/fence/lock/genesis/history基础矩阵及 no-owner→claim→draft-bound→prepared→receipt→intent→commit edge crash；draft覆盖 response loss、stale-zero/分页/权限/API/nonce/exhaustive-negative/upload edges；四种 generated PR kind分别覆盖 ref-create/push/PR-create/bound-append response loss，唯一 draft/open/closed/merged/queued match，>1页重复/乱序/partial、nonce metadata spoof/wrong creator/fork head/same patch different base，普通 zero后迟到 PR、cancel-vs-create/bind/queue/merge、旧 fence late bind/merge，original/replacement response-loss/并存/多级/reopen/ref ABA/stale check/ruleset bypass；以及 post-intent、permissions、六类 blocked。Covers: B-001–B-031. Owner: integration-test owner. Depends on: SP700-T2–SP700-T11。Done when:每个 mutation断言 ownership/fence/receipt/history/outcome/PR/marker/release/row/exit，最多一个 merge-authorized PR/current marker，无 orphan draft/PR、late merge、ownerless Release或漏 row，用户 canaries不变。Verify: `bash tests/test_public_benchmark.sh`; `bash tests/test_behavior_eval.sh`; `bash tests/test_hook_perf_contract.sh`; `bash tests/test_release_workflow.sh`.

- [ ] `SP700-T13` 在同一 immutable implementation head 完成生产回归、安全审查与 current-head evidence：运行 Rust/benchmark/behavior/hook/release/docs/broad checks；independent reviewer 核对 identity/mapping/runtime broker/baseline/ledger、统一锁序、pre-draft claim/binding、四种 generated PR planned/bound response-loss/all-state discovery、genesis/rollover revocation、六类 recovery-blocked，SEC-11 reviewer另审 command execution、trusted App/nonce与 merge-gate/ruleset bypass。Covers: B-001–B-031. Owner: verification owner + independent read-only reviewer + human security reviewer. Depends on: SP700-T12；GH-699 T3–T6；current-head CI/review/thread/merge state。Done when: fresh checks全绿、findings为零，head/CI/0 threads/reviews/merge state一致；merge/release仍按授权。Verify: 记录普通仓库命令与current-head远端证据。
