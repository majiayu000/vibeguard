...
- [ ] `SP700-T10` 接入 strict summary、serialized evidence 与 two-phase publication：summary 绑定全部 required/display-only reports且 digest排除自身；attempts/reconciler/publish 共用 candidate/source-identity、repository-global ledger 与 renewable/fenced publication leases。valid phase 在 lease/CAS 下 de-current、写 receipt、把 pre-approved new-current patch/review/base digest 绑定进 intent并 publish；commit 后持久化 `publication_ownership_record(post_commit_marker_pending)`。PR拒绝/超时/owner消失时 scheduled reconciler 以更高 fence 接管，验 intent/release/zero-marker 后 supersede旧 PR并为同 candidate建 human-reviewed replacement；无获批 replacement 则 attest `marker_recovery_blocked`，短 lease可释放但 durable ownership 阻断下一 candidate，直到 exact marker merge 后 terminal。Covers: B-016, B-017, B-018, B-021, B-022, B-027, B-029, B-031. Owner: release/summary/reconciler owner. Depends on: SP700-T2, SP700-T3, SP700-T5, SP700-T7–T9。Done when: owner crash、PR rejected/closed/stale、deadline/heartbeat、two valid candidates、stale fence/CAS、cancel before/after intent/commit、zero-marker rollback、nonvalid preserve-current矩阵均无无主状态/双 current/后续误发布。Verify: `bash tests/test_public_benchmark.sh`; `bash tests/test_release_workflow.sh`.

- [ ] `SP700-T11` 只从 SP700-T10 验签 summary 生成 README/locale marker：valid publication 前生成 de-current PR和 exact new-current patch，后者须先 human review并由 intent 绑定；commit 后合并 new-current，recovery 只可从同 summary重建 replacement；publish_nonvalid 只加无 current row并保留旧 marker。Covers: B-008, B-015, B-016, B-017, B-018, B-020, B-023, B-031. Owner: docs generator owner. Depends on: SP700-T9, SP700-T10；README human review gate。Done when: new-current approved/merged、rejected、closed、stale-base、deadline takeover、replacement re-review、marker_recovery_blocked，以及 de-current/rollback/nonvalid goldens全通过；任一时刻最多一个 current marker且无无主 zero-marker。Verify: `python3 scripts/ci/render_public_benchmark.py --check`; `bash scripts/ci/validate-doc-paths.sh`; `bash scripts/ci/validate-doc-command-paths.sh`.

- [ ] `SP700-T12` 建立完整 adversarial harness，新增 mapping identity injection、runtime grant/exec-broker、baseline boundary、exact-ref binding、global ledger rewrite、两 valid candidate、expired fence/CAS、receipt/rollback，以及 post-commit owner crash、new-current rejected/closed/stalled、takeover replacement、stale old token、explicit recovery block 与后续 candidate denial 矩阵。Covers: B-001–B-031. Owner: integration-test owner. Depends on: SP700-T2–SP700-T11。Done when:每个 mutation断言 ownership/fence/PR/marker/release/exit，最多一个 current marker、无无主 zero-marker/public partial release，用户 canaries不变。Verify: `bash tests/test_public_benchmark.sh`; `bash tests/test_behavior_eval.sh`; `bash tests/test_hook_perf_contract.sh`; `bash tests/test_release_workflow.sh`.

- [ ] `SP700-T13` 在同一 immutable implementation head 完成生产回归、安全审查与 current-head evidence：运行 Rust/benchmark/behavior/hook/release/docs/broad checks；independent reviewer 核对 identity/mapping/runtime broker/baseline/ledger/exact-ref、publication ownership/fencing、pre-approved marker plan、post-commit takeover/recovery block 与 required-platform gate，SEC-11 reviewer另审 command execution。Covers: B-001–B-031. Owner: verification owner + independent read-only reviewer + human security reviewer. Depends on: SP700-T12；GH-699 T3–T6；current-head CI/review/thread/merge state。Done when: fresh checks全绿、findings为零，head/CI/0 threads/reviews/merge state一致；merge/release仍按授权。Verify: 记录普通仓库命令与current-head远端证据。
