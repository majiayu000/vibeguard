...
- [ ] `SP700-T1` 固化 official benchmark protocol 与 H-001–H-005 产品决策门：除获批 decisions/mappings/required set/release policy/corpus counts 外，钉住 canonical benchmark config、effectiveness initial states、每个 latency surface 的 ordered cases/repetitions/interleaving、每个 executor 的 timeout/termination-grace/stdout/stderr caps、target-specific interpreter 与 production path 全部 external-executable identities、`fresh_per_sample` latency state/initial-state 和 `nearest_rank_v1` estimator（integer ns，Pq=`ceil(q*n/100)-1`，无插值），以及 environment baseline 的 manifest-bound no-op workload/argv/stdin/env、warmup/measurement/interleaving、timer boundary、estimator、threshold 与 inclusive comparison。用户 config、ambient PATH、limit overrides、cumulative sample state 与 runner 自选 estimator/workload 永不进入 official 输入。Covers: B-002, B-003, B-006, B-009, B-011, B-012, B-018, B-024, B-026, B-031. Owner: maintainer decision + protocol owner. Depends on: product/tech review与未决选择批准。Done when: missing/invalid decisions/config/state/schedule/estimator/limits/executable、unknown/reordered case、state reuse、非正 repetition 或 cap 均在零执行前 unavailable；n=1/2/3/20/21 percentile 及 baseline threshold-1/equal/+1、count/order/timer-boundary goldens固定。Verify: `cargo test --manifest-path vibeguard-runtime/Cargo.toml bench::protocol`; `bash tests/test_public_benchmark.sh`.

- [ ] `SP700-T2` 扩展 offline-verifiable release identity、trusted-launcher mapped-image binding 与 handle chain：installer 持久化 signed bundle/manifest/pinned roots/receipt；独立受信的最小 native launcher 在执行 runtime 前验签 manifest、打开并校验 target runtime handle 的 size/SHA-256/metadata，随后才在 Unix 从同一 handle `fexecve/execveat` 并传 identity fd，或在 Windows 从 deny-write/delete handle 启动并持有到 child handshake；child 从 inherited binding handle重算 runtime identity只作 defense in depth，再验证 payload/wrapper/canonical config/interpreter/baseline workload及其它 declared external executables。`current_exe` pathname仅诊断。Covers: B-002, B-013, B-016, B-021, B-024, B-028, B-030. Owner: identity + launcher owner. Depends on: SP700-T1；GH-699 payload。Done when: untrusted/replaced launcher、pre-exec runtime digest mismatch、start-malicious-then-replace-path-with-signed-binary、bundle/receipt双篡改、wrong root/issuer/subject、checksum-only、symlink/metadata race/user config/undeclared executable 均 fail closed；不支持 trusted launcher 或 handle-bound exec 的平台 official unavailable。Verify: `cargo test --manifest-path vibeguard-runtime/Cargo.toml bench::identity`; `bash tests/test_setup.sh`; `bash tests/test_public_benchmark.sh`.

- [ ] `SP700-T3` 在明确的 **schemas/public_benchmark_protocol.schema.json** 建立 protocol ownership，并建立 corpus/truth/mapping/report/summary/failure-manifest/review-record schemas、JCS canonicalizer、maintainer-attested reviewer roster verifier和 published+blocked frontier ledger validator；JSONL digest framing 固定为 schema-valid records 按 UTF-8 case ID 排序后的单一 JCS array；summary preimage 固定为删除 `summary_digest` 字段后的完整 object，detached attestation不入 preimage；release aggregation 对每个 target 使用 present/missing union，pre-attestation interruption 使用 server-authenticated early identity branch；JSON number 仍限 safe integers。Covers: B-004, B-016, B-019, B-022, B-025, B-026, B-027, B-029, B-031. Owner: schema/integrity owner. Depends on: SP700-T1与genesis approval。Done when: Rust/Python JSONL framing与summary golden一致，raw-line/concatenated-JCS/line-order drift、self-placeholder/attestation-in-preimage拒绝；missing target null identity、pre-attestation event/API identity、distinct fake reviewer IDs with same key/subject与blocked-suffix rewrite矩阵全绿；safe integer/ledger mutation矩阵全绿。Verify: `python3 scripts/ci/validate_public_benchmark.py`; `cargo test --manifest-path vibeguard-runtime/Cargo.toml bench::digest`; `bash tests/test_public_benchmark.sh`.

- [ ] `SP700-T4` 在 SP700-T3 schema/roster validator 冻结后补齐五类 production surfaces并 populate corpus/truth/mapping/ledger：每条 ground-truth/mapping/security review record 必须由 roster identity 签名并绑定 artifact digest/role/decision/source commit；不得用多个自报 ID 伪造独立性。Covers: B-003, B-004, B-005, B-006, B-025, B-026, B-027. Owner: detector owners + corpus/mapping single writer + authenticated independent reviewers. Depends on: SP700-T1, SP700-T3与各 detector security gate。Done when:五类 matched pairs、signed review records与released mappings齐全；roster/record/overlap mutation fail；未落地类别 official unavailable。Verify: `python3 scripts/ci/validate_public_benchmark.py`; `cargo test --manifest-path vibeguard-runtime/Cargo.toml bench::mapping`; `bash tests/test_public_benchmark.sh`.
...
- [ ] `SP700-T10` 接入 strict summary、serialized evidence 与 two-phase publication：summary digest排除自身且 detached attest；summary 绑定最终展示的全部 required/display-only reports，但 validity 只消费 required subset；attempts/reconciler/publish共用 lease+watermark。phase A 只创建 private attempt draft、上传并重验全部 assets；phase B 先 attest publish_intent、合并并重验去除旧 current marker 的 human-reviewed PR，再以 draft→published 为唯一 commit point。reconciler 通过 protected environment 只获得精确 draft mutation authority：pre-intent cancel删除对应 draft并记录，post-intent cancel即使 same-tag draft 已存在也只能按匹配 intent 幂等完成/验证；target 与 release-aggregation failure manifest 分别绑定单 target或 present/missing 完整 failed-summary inputs及 candidate ledger identity；staged identity 缺失时只使用受信 workflow event/API 写 pre-attestation branch。Covers: B-016, B-017, B-018, B-021, B-022, B-027, B-029, B-031. Owner: release/summary/reconciler owner. Depends on: SP700-T2, SP700-T3, SP700-T5, SP700-T7–T9。Done when: required/display-only input replacement、summary self-digest mutation、release-wide disagreement/missing-input、partial draft upload、pre-attestation cancel、cancel before/after intent/after commit、matching-vs-mismatched draft guard、blocked-suffix rewrite、stale old-current marker、permission denial、watermark lag与duplicate delivery矩阵各自收敛到唯一合法状态；无未签名展示附件、public partial assets或 stale current marker。Verify: `bash tests/test_public_benchmark.sh`; `bash tests/test_release_workflow.sh`.

- [ ] `SP700-T11` 只从 SP700-T10 验签 summary 内绑定的 required/display-only reports 生成 README/locale marker：publication 前生成只删除旧 current marker 的双 locale PR，commit 后生成添加新 row/current marker 的 PR；每个平台显示 provenance/effectiveness/status/link，每个 protocol-declared latency surface 使用固定独立 P95/status 列或子行，不做 reduction；blocked 无 candidate row，publish_nonvalid 仅同版本空 metric row。Covers: B-008, B-015, B-016, B-017, B-018, B-020, B-023, B-031. Owner: docs generator owner. Depends on: SP700-T9, SP700-T10；README human review gate。Done when: 3×3×terminal、surface add/remove/order、required/display-only summary input/digest/attestation 与 pre-publication no-current/post-publication new-current goldens全通过；任一时刻最多一个 current marker，旧 release 在新 release 公开后绝不保持 current，游离附件、手工数字/列漂移被 freshness拒绝。Verify: `python3 scripts/ci/render_public_benchmark.py --check`; `bash scripts/ci/validate-doc-paths.sh`; `bash scripts/ci/validate-doc-command-paths.sh`.

- [ ] `SP700-T12` 建立完整 adversarial harness，新增 launcher-pre-exec runtime tamper、untrusted launcher、ambient PATH/fake-or-missing `git`/undeclared child exec、executable digest/version drift、timeout/grace/stdout/stderr cap drift/overflow、start-then-replace mapped-image race、privacy raw-vs-free-text、completed-axes clean cancel、cleanup-before-report-seal、nearest-rank vs floor boundary、baseline schedule/estimator/threshold-1/equal/+1/timer-boundary、latency history reset、summary self-preimage、release-wide missing-target null identity、pre-attestation interruption、blocked-ledger-suffix rewrite、old-current marker publication window、fake reviewer IDs/shared key、以及 exact-draft permission/same-tag guard/two-phase cancel-before/after-intent/after-commit；继续覆盖既有 identity/config/schedule/safe-integer/lease/ledger/process-tree矩阵。Covers: B-001, B-002, B-003, B-004, B-005, B-006, B-007, B-008, B-009, B-010, B-011, B-012, B-013, B-014, B-015, B-016, B-017, B-018, B-019, B-020, B-021, B-022, B-023, B-024, B-025, B-026, B-027, B-028, B-029, B-030, B-031. Owner: integration-test owner. Depends on: SP700-T2–SP700-T11。Done when:每个 mutation断言具体 artifact/state/exit，public partial release/stale current marker不存在，用户 canaries不变且所有非valid路径非零。Verify: `bash tests/test_public_benchmark.sh`; `bash tests/test_behavior_eval.sh`; `bash tests/test_hook_perf_contract.sh`; `bash tests/test_release_workflow.sh`.

- [ ] `SP700-T13` 在同一 immutable implementation head 完成生产回归、安全审查与 current-head merge evidence：运行全部 Rust、benchmark、behavior、hook、release、docs 和 broad ordinary contract checks；由 independent reviewer 对照 product/tech/tasks 核对 official identity、production-only mapping、参数数组、protocol-bound limits/all executables/baseline、privacy/cleanup、完整分母、digest split、published+blocked ledger frontier、pre-attestation/release-aggregation failure evidence、README publication ordering与 required-platform gate，SEC-11 reviewer另行确认 command-execution surface。Covers: B-001, B-002, B-003, B-004, B-005, B-006, B-007, B-008, B-009, B-010, B-011, B-012, B-013, B-014, B-015, B-016, B-017, B-018, B-019, B-020, B-021, B-022, B-023, B-024, B-025, B-026, B-027, B-028, B-029, B-030, B-031. Owner: verification owner + independent read-only reviewer + human security reviewer. Depends on: SP700-T12；GH-699 T3–T6 merged/no-clone launcher evidence for B-001 official claim；current-head CI、independent review、review threads 与 clean merge state。Done when: fresh deterministic checks 全绿，reviewer/security findings 为零，PR head SHA、required CI、0 unresolved actionable threads、independent review 与 merge state 互相一致；merge/release 仍只按当前授权执行。Verify: 运行本文件“验证”中的普通仓库命令并记录 current-head CI/reviewer/thread/merge-state 结果。
