...
- [ ] `SP700-T3` 在明确的 **schemas/public_benchmark_protocol.schema.json** 建立 protocol ownership，并建立 corpus/truth/mapping/report/summary/failure-manifest/review-record/publication-history schemas、JCS canonicalizer、reviewer verifier与 corpus-ledger validator；corpus ledger/publication history 分域。publication canonical frontier 为 `(repo_node_id,length,root,full_prefix_digest)`：length-zero genesis仅在 store 无 head时初始化，之后 reader 取得签名单调 current head、重放 prefix精确至 head；append 原子比较完整 expected frontier + current fence并返回签名 successor。versioned closed history-record union使用 `jcs-rfc8785-v1` digest，覆盖 owner-claim/draft-binding/prepared/de-current plan|binding|revocation/receipt/intent/commit/takeover/六类 recovery-blocked/recovered/terminal，绑定 prior phase/frontier与 expected/new fence，deterministic fold 唯一导出状态并拒绝非法 transition。mapping 只接受 asset logical IDs；JSONL 固定 case-ID-sorted JCS array；summary preimage 删除 `summary_digest`；release target使用 present/missing union，pre-attestation interruption使用 server-authenticated early identity；JSON number限 safe integers。Covers: B-004, B-016, B-019, B-022, B-025, B-026, B-027, B-029, B-031. Owner: schema/integrity owner. Depends on: SP700-T1与genesis approval。Done when: canonical framing/summary goldens、mapping/reviewer/target/early identity、blocked-suffix rewrite、history unknown-version/digest/tail-truncation/fork/illegal transition/predecessor-bypass与expired-fence competing append矩阵全绿。Verify: `python3 scripts/ci/validate_public_benchmark.py`; `cargo test --manifest-path vibeguard-runtime/Cargo.toml bench::digest`; `bash tests/test_public_benchmark.sh`.
...
- [ ] `SP700-T10` 接入 strict summary 与统一 publication state machine：所有 actors 使用 `repo_node_id`，按 source/candidate→ledger→publication→branch CAS；首次 Release API/PR mutation前 CAS `owner_claimed`，draft create response后/upload前 `draft_bound`，重验后 `prepared`。history 验证签名 current head/full prefix并对完整 frontier+fence CAS；genesis 由 history 而非 corpus ledger 证明无历史 valid publication。rollover PR 使用 planned/bound phase及 latest-frontier merge gate；pending取消须 revoke gate/queue/PR/head并取得不可再 merge receipt，已 merge才 rollback；pending replacement先撤销旧 PR再用新 fence/head/PR fresh review。valid/nonvalid 均先 CAS `intent_written`，它是两类 Release commit state 的唯一 predecessor。reconciler least permissions为 `actions: read`/`contents: write`/`pull-requests: write`。claim/draft/pending/merged pre-intent分支只可证明 cleanup/revoke/rollback，不可 publish；post-intent matching Release→verify+README，否则 matching intent-bound draft→publish+README，二者只写 `recovered_publication`；皆无→`release_recovery_blocked`。draft/de-current/rollback/marker/nonvalid failures进入六类 blocked并保留 owner；只有 authenticated exhaustive negative receipt或 exact cleanup/revocation/rollback/Release+README completion才 terminal。Covers: B-016, B-017, B-018, B-021, B-022, B-027, B-029, B-031. Owner: release/summary/reconciler owner. Depends on: SP700-T2, SP700-T3, SP700-T5, SP700-T7–T9。Done when: claim-before-mutation、create-response loss/stale-zero/partial-pagination/API-denial/exhaustive-negative、draft upload edges、pending PR plan/bind/replacement/cancel-vs-merge/reopen/ABA/late-merge/ruleset bypass、history predecessor、post-intent truth table、permissions、六类 blocked、two candidates及 lock/fence/CAS矩阵均无 orphan/late merge/双 current/漏 row。Verify: `bash tests/test_public_benchmark.sh`; `bash tests/test_release_workflow.sh`.

- [ ] `SP700-T11` 只从 SP700-T10 owner-bound summary 生成 exact README/locale patches：genesis 用 zero-marker receipt且不建 no-op de-current PR；rollover 的 de-current PR 先 plan/bind且受 current-owner merge gate约束，取消须 revocation receipt；已 merge才生成 reviewed rollback；valid intent 绑定 reviewed new-current，nonvalid intent 绑定 reviewed unmarked row。所有 original/replacement PR 必须同 summary、同 patch intent并重新 human review。Covers: B-008, B-015, B-016, B-017, B-018, B-020, B-023, B-031. Owner: docs generator owner. Depends on: SP700-T9, SP700-T10；README human review gate。Done when: genesis、rollover pending cancel/merge、rollback/new-current/nonvalid-row及 reopen/queue/ABA/replacement、六类 recovery-blocked goldens全通过；最多一个 merge-authorized PR/current marker且无 ownerless Release。Verify: `python3 scripts/ci/render_public_benchmark.py --check`; `bash scripts/ci/validate-doc-paths.sh`; `bash scripts/ci/validate-doc-command-paths.sh`.

- [ ] `SP700-T12` 建立完整 adversarial harness，新增 mapping identity injection、runtime broker、baseline boundary、`repo_node_id` exact-ref、global ledger rewrite、fenced atomic competing append、固定/反向锁序、genesis/history/rollover、两 candidate、expired fence/CAS，以及 no-owner→claim→draft-bound→prepared→receipt→intent→commit 每条 edge crash与 predecessor bypass；覆盖 create response loss、前 N 次 stale-zero后迟到 exact draft、partial pagination/permission trim/rate-limit/5xx/nonce歧义、authenticated exhaustive negative、每个 upload edge；pending de-current PR覆盖 plan/create/bind/review/queue、reject/close/stall/drift replacement、original/replacement并存、replacement crash、cancel-vs-merge、late merge、reopen/ref ABA/stale check/ruleset bypass；以及 post-intent exclusivity、permission denial、六类 recovery block 与后续 denial。Covers: B-001–B-031. Owner: integration-test owner. Depends on: SP700-T2–SP700-T11。Done when:每个 mutation断言 ownership/fence/receipt/history/outcome/PR/marker/release/row/exit，最多一个 merge-authorized PR/current marker，无 orphan draft、late de-current merge、ownerless Release或漏 row，用户 canaries不变。Verify: `bash tests/test_public_benchmark.sh`; `bash tests/test_behavior_eval.sh`; `bash tests/test_hook_perf_contract.sh`; `bash tests/test_release_workflow.sh`.

- [ ] `SP700-T13` 在同一 immutable implementation head 完成生产回归、安全审查与 current-head evidence：运行 Rust/benchmark/behavior/hook/release/docs/broad checks；independent reviewer 核对 identity/mapping/runtime broker/baseline/ledger、统一锁序、pre-draft claim、draft binding、genesis/rollover pending/revocation、三类 post-release PR takeover及六类 recovery-blocked，SEC-11 reviewer另审 command execution与 merge-gate/ruleset bypass。Covers: B-001–B-031. Owner: verification owner + independent read-only reviewer + human security reviewer. Depends on: SP700-T12；GH-699 T3–T6；current-head CI/review/thread/merge state。Done when: fresh checks全绿、findings为零，head/CI/0 threads/reviews/merge state一致；merge/release仍按授权。Verify: 记录普通仓库命令与current-head远端证据。
