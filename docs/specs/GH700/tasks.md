...
- [ ] `SP700-T10` 接入 strict summary与统一 publication client/machine，只消费 T3 manifest-pinned mTLS authority API，不得拥有/替换 backend、直接开 DB或降级 local store；reconciler GitHub token仅 `actions/contents/pull-requests: read`，所有 target write credential仅 authority/broker持有。每个 Release write在 call前持久化 planned slot+guard；uncertain禁止重发，按 exact discovery recover-bind/not-applied/compensate/block；`draft_create` blocked precedence固定为 `draft_recovery_blocked`。phase gate只接受 `{bound,not_applied,compensated}` 且 intended effect闭合；takeover引用旧 plan fresh authorize。publication actor遇 governance suffix保留 phase并重规划；post-invalidation fold接受验签 governance suffix与 authenticated terminal no-publication chain，但拒绝伪造/跨 candidate/非终态/current restoration/其它 owner。Covers: B-016–B-018, B-021, B-022, B-027, B-029, B-031. Owner: publication client owner. Depends on: T2/T3/T5/T7–T9。Done when:wrong endpoint/socket/transport/version/server/client identity、ambient discovery、write token exposure、六 kind crash/recovery/takeover均 fail closed，authority unavailable时零 remote mutation。Verify: `bash tests/test_public_benchmark.sh`; `bash tests/test_release_workflow.sh`.
...
- [ ] `SP700-T12` 建立完整 adversarial harness：rotation覆盖 predecessor/ordinal/epoch无环构造、suffix重签 certificate与 successor root/full-prefix自引用拒绝；authority API覆盖 endpoint/socket closed union、transport/version、server/client identity/policy、redirect/ambient trust；reconciler write-token/credential leak拒绝；external anchor覆盖 stale/equal epoch、older/forked prefix、missing anchor及 valid threshold restore；persistence inventory逐项删除/tamper fail。post-invalidation suffix接受三种 governance及 terminal no-publication/takeover chain并拒绝伪造/非终态/跨 candidate/current restoration。六 Release kind覆盖 crash/0/1/N/in-flight/partial/compensation/takeover，`draft_create` generic blocked-kind拒绝；mutation capsule覆盖 no-cycle/placeholder/raw-secret/send-once；invalidation覆盖 future SHA/post-merge；guard覆盖 install/live。Covers: B-001–B-031. Owner: integration-test owner. Depends on: T2–T11。Done when:所有 slot effect closure成立，最多一 draft/public Release/exact assets。Verify: `bash tests/test_public_benchmark.sh`; `bash tests/test_behavior_eval.sh`; `bash tests/test_hook_perf_contract.sh`; `bash tests/test_release_workflow.sh`.
