...
- [ ] `SP700-T3` 作为唯一 publication-authority store/deployment与 restore-anchor implementation single writer，建立 [publication_history_contract.md](publication_history_contract.md) 指定的 `publication_authority_sqlite_v1` 与独立账户 `publication_restore_anchor_dynamodb_v1` backends、schema/fold、deploy/bootstrap/migrate/recover/restore。实现路径固定为 `vibeguard-runtime/src/publication_authority/{mod.rs,store.rs,broker.rs,recovery.rs,restore_anchor.rs}`+main CLI，部署/初始化路径固定为 planned **schemas/publication_authority_deployment.schema.json**+**.github/workflows/publication-authority-deploy.yml**+**scripts/ci/bootstrap_publication_authority.py**及 planned **.github/workflows/publication-restore-anchor-deploy.yml**+**scripts/ci/bootstrap_publication_restore_anchor.py**；T3独占同时必需、共享 authority/policy identity 的 manifest-pinned HTTPS-mTLS `client_api`与 Unix-peercred `control_api`，以及 durable volume/DB/process-lock、SQLite WAL/FULL fsync/closed indexes+capsules+outbox+send-once inventory、DynamoDB endpoint/table identity/SigV4 short-lived role/immutable epoch+HEAD conditional CAS proof和 restore execution，独立 maintainer governance仅持 threshold approval/anchor admin role，T10只可调认证 client API且两者均不得越权。`publication_history` frontier唯一为 `(repo_node_id,history_length,history_root,full_prefix_digest)`，top-level discriminator唯一为 `record_kind`；八个 blocked variants exact 为 `{release_mutation_recovery_blocked,draft_recovery_blocked,decurrent_pr_recovery_blocked,rollback_recovery_blocked,marker_recovery_blocked,nonvalid_row_recovery_blocked,invalidation_recovery_blocked,release_recovery_blocked}`。history authorization分 publication owner domain与 owner-free governance domain；rotation stable approval与 per-append certificate分离，certificate仅绑定 predecessor+successor ordinal/next epoch且不含 successor root/full-prefix；leaf/root/revocation使用独立 governance lease/fence/threshold，phase-neutral且两域 fence隔离。`post_invalidation_zero` suffix接受 terminal non-valid/current prepared owner、以 `publication_terminal_no_publication` 结束的 authenticated same-candidate chain及三种 governance records。六种 Release `mutation_kind={draft_create,draft_update,draft_delete,asset_upload,asset_delete,publish}` 使用 closed slot machine；`draft_create`不可证明 outcome只进 `draft_recovery_blocked`，generic blocked kind仅其余五种。每种只持久化唯一 `mutation_nonce_digest`、opaque capsule/delivery与 template/effective digests；invalidation planned禁 future merge fields。Covers: B-004, B-016, B-019, B-022, B-025–B-027, B-029, B-031. Owner: schema/integrity + publication-authority/restore-anchor implementation owner；bootstrap/migration/restore execution由T3，threshold approval/anchor admin由独立 maintainer governance。Depends on: T1。Done when:共用 schema/goldens覆盖无环 rotation cutover、双 API shared-identity/policy drift、draft blocked precedence、terminal chain/governance suffix、closed persistence inventory及 external anchor table replacement/并发 single-winner/stale CAS/ack-loss/DB↔anchor crash/threshold restore；真实 durable-volume+隔离 DynamoDB test-table fixture证明无丢失/重复/unsigned frontier且 mock不算通过。Verify: `python3 scripts/ci/validate_public_benchmark.py`; `cargo test --manifest-path vibeguard-runtime/Cargo.toml publication_authority`; `bash tests/test_public_benchmark.sh`.
...
- [ ] `SP700-T10` 接入 strict summary与统一 publication client/machine，只消费 T3 manifest-pinned mTLS `client_api`，不得调用 Unix `control_api`、拥有/替换 backend、直接开 DB或降级 local store；reconciler GitHub token仅 `actions/contents/pull-requests: read`，所有 target write credential仅 authority/broker持有。每个 Release write在 call前持久化 planned slot+guard；uncertain禁止重发，按 exact discovery recover-bind/not-applied/compensate/block；`draft_create` blocked precedence固定为 `draft_recovery_blocked`。phase gate只接受 `{bound,not_applied,compensated}` 且 intended effect闭合；takeover引用旧 plan fresh authorize。publication actor遇 governance suffix保留 phase并重规划；post-invalidation fold接受验签 governance suffix与 authenticated terminal no-publication chain，但拒绝伪造/跨 candidate/非终态/current restoration/其它 owner。Covers: B-016–B-018, B-021, B-022, B-027, B-029, B-031. Owner: publication client owner. Depends on: T2/T3/T5/T7–T9。Done when:wrong client endpoint/transport/version/server/client identity、response shared-authority/policy drift、control method调用、ambient discovery、write token exposure、六 kind crash/recovery/takeover均 fail closed，authority unavailable时零 remote mutation。Verify: `bash tests/test_public_benchmark.sh`; `bash tests/test_release_workflow.sh`.
...
- [ ] `SP700-T12` 建立完整 adversarial harness：rotation覆盖 predecessor/ordinal/epoch无环构造、suffix重签 certificate与 successor root/full-prefix自引用拒绝；authority API覆盖 simultaneous client/control API、shared authority/policy drift、transport/version/server/client/peer identity、redirect/ambient trust；reconciler write-token/credential leak拒绝；external anchor在隔离 DynamoDB test table覆盖 table replacement、stale/equal epoch、older/forked prefix、并发 single-winner、ack-loss read-confirm、DB↔anchor crash、bootstrap replay及 valid threshold restore，mock不计通过；persistence inventory逐项删除/tamper fail。post-invalidation suffix接受三种 governance及 terminal no-publication/takeover chain并拒绝伪造/非终态/跨 candidate/current restoration。六 Release kind覆盖 crash/0/1/N/in-flight/partial/compensation/takeover，`draft_create` generic blocked-kind拒绝；mutation capsule覆盖 no-cycle/placeholder/raw-secret/send-once；invalidation覆盖 future SHA/post-merge；guard覆盖 install/live。Covers: B-001–B-031. Owner: integration-test owner（只写 fixture/test，不拥有 backend/deploy/credential）。Depends on: T2–T11。Done when:所有 slot effect closure成立，最多一 draft/public Release/exact assets。Verify: `bash tests/test_public_benchmark.sh`; `bash tests/test_behavior_eval.sh`; `bash tests/test_hook_perf_contract.sh`; `bash tests/test_release_workflow.sh`.

- [ ] `SP700-T13` 在同一 immutable head完成回归/evidence；independent reviewer核对无环 rotation cutover、authority API trust、external restore anchor、closed persistence inventory、draft blocked precedence、governance/domain-fence与六 mutation recovery；SEC-11审 read-only reconciler、authority/broker-only write credential、outbox/compensation、trusted App/nonce/KMS/OS issuer与 command execution。Covers: B-001–B-031. Owner: verification owner + independent reviewer + human security reviewer. Depends on: T12；GH-699 T3–T6；current-head gates。Done when:fresh checks、CI、0 threads/reviews/merge state一致；merge/release仍按授权。Verify: current-head证据。
