...
- [ ] `SP700-T10` 接入 strict summary、serialized evidence 与统一 publication state machine：所有 actors 使用 server-authenticated `repo_node_id`，严格按 source/candidate→repository ledger→repository publication→branch CAS 取短 lease；在任何 marker/Release/README 可见动作前写 `publication_ownership_record(prepared)`。valid 闭集区分 ledger/CAS 证明的 `genesis_zero` 与单 current 的 `rollover_one`；genesis 只排除 exact current `(repo_node_id,candidate,run,attempt,fence)` prepared owner，任何其它 active owner 均拒绝。intent 绑定 receipt + pre-approved new-current patch；publish_nonvalid intent 绑定 reviewed unmarked-row patch。prepared owner 必须覆盖 commit/状态写间 crash、pre-intent rollback、post-commit marker与nonvalid row；PR拒绝/关闭/超时/owner消失统一由 higher-fence reconciler 创建同 candidate exact reviewed replacement，无 replacement则对应 `*_recovery_blocked` 并阻断下一 publication，exact rollback/delete或README merge后才 terminal。Covers: B-016, B-017, B-018, B-021, B-022, B-027, B-029, B-031. Owner: release/summary/reconciler owner. Depends on: SP700-T2, SP700-T3, SP700-T5, SP700-T7–T9。Done when: genesis self-owner通过/other-owner拒绝、no-op禁止、owner crash at every state edge、rollback/new-current/nonvalid-row rejected/closed/stale/deadline、two candidates、reverse-lock deadlock、stale fence/CAS、repo-name/node-ID drift矩阵均无无主状态/双 current/漏 row。Verify: `bash tests/test_public_benchmark.sh`; `bash tests/test_release_workflow.sh`.
...
- [ ] `SP700-T12` 建立完整 adversarial harness，新增 mapping identity injection、runtime broker、baseline boundary、`repo_node_id` exact-ref、global ledger rewrite、固定/反向锁序、genesis与rollover、两 candidate、expired fence/CAS，以及 prepared→transition/intent/commit 每条 edge crash、rollback/new-current/nonvalid-row rejected/closed/stalled、replacement review、三类 recovery block 与后续 denial 矩阵。Covers: B-001–B-031. Owner: integration-test owner. Depends on: SP700-T2–SP700-T11。Done when:每个 mutation断言 ownership/fence/PR/marker/release/row/exit，最多一个 current marker、无无主 zero-marker/public Release或漏 row，用户 canaries不变。Verify: `bash tests/test_public_benchmark.sh`; `bash tests/test_behavior_eval.sh`; `bash tests/test_hook_perf_contract.sh`; `bash tests/test_release_workflow.sh`.
