...
- [ ] `SP700-T3` 在明确的 **schemas/public_benchmark_protocol.schema.json** 建立 protocol ownership，并建立 corpus/truth/mapping/report/summary/failure-manifest/review-record/publication-history schemas、JCS canonicalizer、reviewer verifier与 corpus-ledger validator。publication frontier 为 `(repo_node_id,length,root,full_prefix_digest)`；history transition分为 fence-independent immutable intent、mutable append authorization envelope与 store-signed committed envelope/receipt。op ID/intent绑定 repo/owner-generation/run/slot/predecessor/kind/payload且 schema拒绝 authorization fence；request另携 current fence/lease/actor。store先查事务 repo+op unique index：same digest返原 receipt，异 digest冲突；absent才验 current generation/fence/predecessor并 append/sign；首次 claim特例仅在 exact fold无 active owner、current lease/fence/run-plan有效且 generation全局未用时原子创建，并将 authenticated-channel 的 store/HSM CSPRNG 256-bit one-time nonce验 typed-JCS domain-separated digest/全局非复用后同事务 KMS封装为 envelope-bound secret capsule；intent/history/log不含原文。capsule unwrap须 current publication lease token/scope+latest frontier+matching server-auth actor，公开 fence/ID不授权；capsule/key保留到整个 successor chain terminal+approved audit window。heartbeat intent仅含 generation/sequence/policy，server envelope认证 accepted-at/lease-expiry/actual fence，fold只信 store time。fold/merge gate只消费 intent owner-generation + committed-envelope actual fence。ack-loss：committed op返原 receipt；uncommitted old fence失败，同 generation续租后用 same intent+new fence重授权；advanced禁止 rebase并按 suffix恢复或 new predecessor/generation/slot重规划；takeover前未提交旧 generation永久失效。versioned JCS union覆盖所有 record与六类 blocked，fold拒绝 ABA。Covers: B-004, B-016, B-019, B-022, B-025, B-026, B-027, B-029, B-031. Owner: schema/integrity owner. Depends on: T1。Done when:每种 record 的 commit前 timeout/commit后 ack-loss/restart+renewed-fence、length-zero/terminal首次 claim、active/concurrent/reused-generation/stale-fence claim、claim后/create前 restart、nonce低熵/重复/dictionary/domain或边界编码错/capsule访问/密文-digest/key-version/rotation/KMS/log-leak、stolen-fence/wrong-actor-right-fence、old-generation terminal后 same-candidate takeover recovery、immutable-intent fence/time/nonce-plaintext injection、gate actual-fence source、heartbeat sequence/TTL/cadence/clock-skew/receipt、heartbeat-takeover两种竞态、old/new fence same-op/receipt、concurrent fences、same-ID digest冲突、takeover-before/after append、terminal/replan suffix、generation/fence reuse、index/receipt/envelope tamper、truncate/fork矩阵全绿。Verify: `python3 scripts/ci/validate_public_benchmark.py`; `cargo test --manifest-path vibeguard-runtime/Cargo.toml bench::digest`; `bash tests/test_public_benchmark.sh`.
...
- [ ] `SP700-T10` 接入 strict summary 与统一 publication state machine：所有 actors 使用 `repo_node_id`，按 source/candidate→ledger→publication→branch CAS；每次 history append遵循 T3 fence-independent intent + current-fence authorization，restart续租只更换 request envelope，不改 op/intent bytes。首次 Release API/PR mutation前以 no-active/terminal predecessor原子 CAS fresh-generation owner claim并同事务封装 recovery nonce capsule；claim后/create前 restart或合法同-candidate takeover必须从 capsule复用 same nonce，失败则 blocked而非创建第二个 draft。等待人工 review按 approved bounded policy持续 append store-timestamped heartbeat，expiry后才允许 higher-fence takeover，二者按 exact-frontier CAS线性化。draft response后/upload前 bind，重验后 prepared。四类 original/replacement PR在首次 ref/commit/PR mutation前 planned，response后 bound；lost response枚举 all states+ref，active/merged/closed分别 bind/恢复/revoke，ordinary zero保owner且不重发，仅线性化 exhaustive-negative证明 absence，其它按 kind blocked。required documentation surface闭集逐 surface绑定 base blob/renderer；rollover须每个 surface恰一条同-version current row并原子更新，genesis须逐 surface全零，mixed/locale drift/缺失重复额外 marker blocked。latest-frontier gate/revoke receipt与 fresh fence/head/nonce/review拒绝 late bind/merge/reopen/ABA。intent_written是两类 commit state唯一 predecessor；reconciler仅 actions/read+contents/pulls write，六 blocked保留 owner。Covers: B-016–B-018, B-021, B-022, B-027, B-029, B-031. Owner: publication owner. Depends on: T2/T3/T5/T7–T9。Done when:所有 record ack-loss old/new fence、initial/concurrent claim、claim-to-create restart/nonce capsule/KMS rotation、per-surface 0/1/version矩阵、heartbeat ack-loss/sequence/TTL/clock-skew、heartbeat-vs-takeover两序、takeover generation及 draft/PR API response-loss/all-state/stale-zero/cancel/merge/replacement/ABA矩阵均无 stale append/duplicate transition/orphan/late merge/跨 surface双 current/漏 row。Verify: `bash tests/test_public_benchmark.sh`; `bash tests/test_release_workflow.sh`.

- [ ] `SP700-T11` 只从 SP700-T10 owner-bound summary 生成 exact README/locale patches：protocol闭集内每个 required documentation surface必须同 release/version/summary；genesis逐 surface zero-marker receipt且不建 no-op de-current PR；de-current/rollback/new-current/nonvalid-row及其 replacements 全部使用一个原子更新全 surface的统一 planned/bound envelope、受保护 nonce、all-state discovery与 latest-frontier merge gate，任何 response-loss不得创建第二个 PR或复用旧 review/check。de-current 取消须 revocation receipt；已 merge才 rollback。Covers: B-008, B-015, B-016, B-017, B-018, B-020, B-023, B-031. Owner: docs generator owner. Depends on: SP700-T9, SP700-T10；README human review gate。Done when:per-surface genesis/rollover、mixed 0/1、missing/duplicate/extra marker、locale version/summary drift、四 kind response-loss/all-state match、cancel/merge/replacement/reopen/queue/ABA/late-merge、六类 blocked goldens全通过；每个 required surface最多一个且全局同版本 current row，无 orphan/ownerless Release。Verify: `python3 scripts/ci/render_public_benchmark.py --check`; `bash scripts/ci/validate-doc-paths.sh`; `bash scripts/ci/validate-doc-command-paths.sh`.

- [ ] `SP700-T12` 建立完整 adversarial harness，覆盖每种 history record 的 commit前 timeout、commit后 ack-loss、restart+renewed fence、首次 claim 的 length-zero/terminal/active/concurrent/reused-generation/stale lease|fence|predecessor、claim-commit后/draft-create前 restart、nonce低熵/dictionary/domain或JCS边界编码错/全局复用/未消费 issuance、capsule缺失/越权/cross-repo|candidate|generation/stolen-public-fence/wrong-actor-right-fence/密文-digest-key篡改/KMS轮换不可用或旧 generation terminal早删/原文注入泄漏/same-candidate takeover recovery、immutable-intent fence/time-field injection、gate误读 intent fence、required doc surfaces逐 surface 0/1/mixed/missing/duplicate/extra/version-summary drift/base-blob CAS、heartbeat多次长 review/ack-loss/duplicate与 digest冲突/forged client clock/TTL-cadence-max-extension/terminal后拒绝/store receipt tamper、heartbeat-vs-takeover两种先后与 takeover后 late heartbeat、rejection/worker-loss在 expiry 前不得 new-generation takeover、uncommitted old-fence fail/new-fence same-op once、committed old/new-fence same receipt、old/new并发、same-ID digest冲突、takeover-before/after append、0/1/N terminal/replan/incompatible suffix、owner-generation/fence reuse、index/receipt/envelope tamper、fork/truncate；draft覆盖 API loss/stale-zero/分页/权限/nonce/exhaustive-negative/upload；四 PR kind覆盖 ref/push/create/bind loss、all-state/multipage/spoof/wrong creator/fork、迟到 PR、cancel/late bind/merge、replacement/multilevel/reopen/ref ABA/stale gate；以及 post-intent/permissions/六 blocked。Covers: B-001–B-031. Owner: integration-test owner. Depends on: T2–T11。Done when:所有 mutation/append 断言 unique fold/actual-fence authorization/store-auth liveness/secret recovery/receipt/outcome，最多一个 active owner/claim nonce/draft/merge-authorized PR/跨 surface同版本 current marker set，无 stale append/duplicate transition/orphan/late merge/ownerless Release/漏 row。Verify: `bash tests/test_public_benchmark.sh`; `bash tests/test_behavior_eval.sh`; `bash tests/test_hook_perf_contract.sh`; `bash tests/test_release_workflow.sh`.

- [ ] `SP700-T13` 在同一 immutable implementation head 完成生产回归、安全审查与 current-head evidence：运行 Rust/benchmark/behavior/hook/release/docs/broad checks；independent reviewer核对 identity/ledger、所有 history append ack-loss/idempotency、atomic initial claim、nonce-capsule restart/takeover recovery、store-auth heartbeat/takeover、pre-draft claim/binding、四 PR planned/bound response-loss/all-state discovery、rollover revocation及六 blocked；SEC-11另审 command execution、trusted App/nonce、KMS capsule/access/log redaction、store receipt/index/time/fence与 merge-gate/ruleset bypass。Covers: B-001–B-031. Owner: verification owner + independent reviewer + human security reviewer. Depends on: T12；GH-699 T3–T6；current-head remote gates。Done when: fresh checks全绿、findings为零，head/CI/0 threads/reviews/merge state一致；merge/release仍按授权。Verify: 记录普通仓库命令与current-head证据。
