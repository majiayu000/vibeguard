...
- [ ] `SP700-T9` 建立唯一 BenchReport、closed raw evidence 与 3×3×terminal outcome contract：先算 axis candidate；`interrupted`（即使两轴已valid且cleanup成功）和 process-tree/report/schema/cleanup failures 都 override 为 inconclusive/nonzero/blank headline。Covers: B-006, B-008, B-014, B-015, B-019, B-020, B-022, B-023, B-025. Owner: report/renderer owner. Depends on: SP700-T7, SP700-T8。Done when: completed-axes-then-clean-cancel golden 与所有 failure matrix不可能 exit0；privacy只剔除free text/payload/stderr而保留closed raw fields。Verify: `cargo test --manifest-path vibeguard-runtime/Cargo.toml bench::render`; `bash tests/test_public_benchmark.sh`.

- [ ] `SP700-T10` 接入 strict summary、serialized evidence 与 two-phase publication：summary digest排除自身且 detached attest；attempts/reconciler/publish共用 lease+watermark。phase A 只创建 private attempt draft、上传并重验全部 assets；phase B 先 attest publish_intent再以 draft→published 为唯一 commit point。pre-intent cancel删除draft并记录；post-intent cancel幂等完成/验证；public release只能是intent绑定的完整状态。Covers: B-016, B-018, B-021, B-022, B-027, B-029, B-031. Owner: release/summary/reconciler owner. Depends on: SP700-T2, SP700-T3, SP700-T5, SP700-T7–T9。Done when: summary self-digest mutation、partial draft upload、cancel before/after intent/after commit、watermark lag与duplicate delivery矩阵各自收敛到唯一合法状态；无 public partial assets。Verify: `bash tests/test_public_benchmark.sh`; `bash tests/test_release_workflow.sh`.
