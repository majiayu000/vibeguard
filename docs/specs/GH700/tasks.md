...
- [ ] `SP700-T1` 固化 official benchmark protocol 与 H-001–H-006 产品决策门：除获批 decisions/mappings/required set/release policy/corpus counts 外，钉住 H-006 唯一 maintainer approval evidence与 `ttl=3600s/period=900s/min-renewal=300s/max-age=604800s` proposal、canonical config、effectiveness states、latency schedule、limits、registry executable闭包、fresh state/estimator/baseline。latency protocol固定 `production_direct_v1`、production topology、`benchmark_only_interposition=[]`、untimed closure receipt及 shipped-production kernel `timed_exec_guard_v1` policy/OS-attestation issuer/completeness；required documentation surface protocol只含 stable id/path/grammar/renderer/schema，mutable base属于 attempt plan。Covers: B-002, B-003, B-006, B-009, B-011, B-012, B-018, B-024, B-026, B-031. Owner: maintainer decision + protocol owner. Depends on: product/tech review与六项选择批准。Done when: missing/unapproved decision/config/schedule/closure/production-equivalent guard在零执行前 unavailable；percentile/baseline、brokered-timed、stable-surface/mutable-base goldens固定。Verify: `cargo test --manifest-path vibeguard-runtime/Cargo.toml bench::protocol`; `bash tests/test_public_benchmark.sh`.
...
- [ ] `SP700-T3` 建立 protocol/corpus/truth/mapping/report/summary/failure/review/publication-history schemas、JCS/verifiers/ledger validator。history新增 digest-pinned trust-bundle、leaf/root rotation、tag identity/completed/finalization guard receipts、stable surface+attempt plan、`invalidate_current`/post-invalidation receipt及第七 blocked；genesis bundle digest/repo/purpose/frontier须由 store/history外 release-identity root验签的 maintainer-governance attestation锚定，leaf由 root轮换，root须 old+new threshold co-sign及独立 governance attestation，历史材料可验证，禁 TOFU/self-authorization。frontier仍用 fence-independent intent+current-fence request/unique op index/store-signed envelope/atomic claim/KMS capsule；H-006 sequence 1从 claim accepted-at计时。Covers: B-004, B-016, B-019, B-022, B-025, B-026, B-027, B-029, B-031. Owner: schema/integrity owner. Depends on: T1。Done when:initial bundle substitution/missing approval、rotation trust failures、H-006 seq1 +299拒绝/+300接受、completed/finalization receipt与 surface-plan tamper、七 blocked均全绿。Verify: `python3 scripts/ci/validate_public_benchmark.py`; `cargo test --manifest-path vibeguard-runtime/Cargo.toml bench::digest`; `bash tests/test_public_benchmark.sh`.
...
- [ ] `SP700-T6` 实现隔离 sandbox、readonly snapshot、fresh state、process-tree cancellation与 closure verifier：effectiveness/untimed conformance由 broker pre-exec deny；timed latency只消费 closure receipt并跑 production direct topology，其 shipped kernel `timed_exec_guard_v1`在 timer前加载 immutable allowlist、image前拒绝、tree后由 trusted OS issuer签完整 event/root receipt，event loss/overflow fail closed。Covers: B-005, B-009, B-010, B-013, B-014, B-024, B-025, B-030. Owner: sandbox/executor owner. Depends on: T1/T2/T4/T5。Done when:仅 timed run触发 undeclared child、event loss、closure/guard tamper均整批无 headline sample；broker 50ms不进入而 production wrapper/guard 50ms进入；无 production-equivalent guard的平台 unavailable。Verify: `cargo test --manifest-path vibeguard-runtime/Cargo.toml bench::sandbox`; `cargo test --manifest-path vibeguard-runtime/Cargo.toml bench::runner`; `bash tests/test_public_benchmark.sh`.
...
- [ ] `SP700-T8` 实现 `bench_case_e2e_ms`：untimed conformance取得 closure receipt后，按 fixed schedule/fresh state以 `production_direct_v1`启动 shipped wrapper与同 topology kernel timed guard；timer内 `benchmark_only_interposition=[]`且不 baseline-subtract，tree封口后 report绑定 topology/mode/closure+OS guard receipts及 raw ns stats。Covers: B-002, B-008, B-011, B-012, B-023, B-024, B-030. Owner: latency owner. Depends on: T1/T2/T6。Done when:brokered-timed、receipt缺失/篡改、timed-only child/event loss整批无 headline sample，broker overhead与 production wrapper/guard overhead分离。Verify: `cargo test --manifest-path vibeguard-runtime/Cargo.toml bench::latency`; `bash tests/test_hook_perf_contract.sh`; `bash tests/test_public_benchmark.sh`.
...
- [ ] `SP700-T10` 接入 strict summary 与统一 publication state machine：只消费 T3 pinned trust chain，按 source/candidate→ledger→publication→branch CAS；claim绑定 immutable protected tag与 stable-surface attempt plan，H-006用 store time线性化 takeover。所有 Release mutation由 sole guard做 pre/post读取，仅 postcheck后签 completed receipt；recovered publication另需新 lease/frontier下的 fresh finalization receipt，任一 tag/ruleset drift按 phase blocked。五类 generated PR planned/bound/all-state recovery；surface protocol不含 mutable base，attempt plan绑定逐 surface mutable tuple，drift重规划。valid支持 rollover/genesis/post-invalidation；fresh post-invalidation receipt绑定 current frontier/invalidation digest/exact prepared owner/current blobs；第五类 invalidation原子更新全 surface并 terminal，七 blocked保留 owner。Covers: B-016–B-018, B-021, B-022, B-027, B-029, B-031. Owner: publication owner. Depends on: T2/T3/T5/T7–T9。Done when:tag每 slot前后及 publish→finalize窗口 races、old receipt after takeover、base drift、valid→invalidation→terminal nonvalid→prepared valid链均无 stale mutation/orphan/跨 surface current。Verify: `bash tests/test_public_benchmark.sh`; `bash tests/test_release_workflow.sh`.

- [ ] `SP700-T11` 只从 owner-bound summary生成 exact README/locale patches：stable surface contract与 mutable attempt plan分离；rollover/genesis/post-invalidation逐 surface证明一致。de-current/rollback/new-current/nonvalid-row/invalidate-current及 replacements使用一个原子全-surface planned/bound envelope、all-state discovery与 latest-frontier gate；invalidation删除 exact marker、标 row invalid并绑定 reason/evidence/merge SHA/before-after blobs。Covers: B-008, B-015–B-018, B-020, B-023, B-031. Owner: docs generator owner. Depends on: T9/T10；human review gate。Done when:五 kind response-loss/replacement/reopen/ABA、valid→invalidation→optional nonvalid→valid、无 receipt/stale frontier/单 locale/receipt mismatch/new current/concurrent CAS与七类 blocked goldens全通过。Verify: `python3 scripts/ci/render_public_benchmark.py --check`; `bash scripts/ci/validate-doc-paths.sh`; `bash scripts/ci/validate-doc-command-paths.sh`.

- [ ] `SP700-T12` 建立完整 adversarial harness：保留 history/claim/nonce/fence/draft/PR矩阵，并加入 initial trust substitution/missing approval及 rotation failures；H-006 seq1 +299拒绝/+300接受、300/900/3600/604800边界；每个 Release slot tag races、response→postcheck drift、publish completed receipt→recovered finalization drift及 takeover旧 receipt；stable surface/base drift；五 PR kind；valid v1→全 surface invalidation→optional terminal nonvalid v2→exact prepared valid v3，fresh post-invalidation receipt及其 missing/stale/locale/mismatch/new-current/concurrent-CAS；untimed closure+production timed guard、broker vs production overhead、仅 timed run late child、event loss/overflow、receipt tamper零 headline sample；以及七 blocked。Covers: B-001–B-031. Owner: integration-test owner. Depends on: T2–T11。Done when:每个 mutation/append unique fold、trusted issuer、store-auth liveness、completed+finalization tag receipts、closure+timed guard receipts，最多一个 owner/draft/merge-authorized PR/current marker set且 benchmark-only overhead不入 headline。Verify: `bash tests/test_public_benchmark.sh`; `bash tests/test_behavior_eval.sh`; `bash tests/test_hook_perf_contract.sh`; `bash tests/test_release_workflow.sh`.

- [ ] `SP700-T13` 在同一 immutable implementation head完成生产回归与 current-head evidence；independent reviewer核对 trust bootstrap/rotations、H-006、completed/finalization tag receipts、stable-surface/attempt-plan、三 valid plans、五 PR与七 blocked；SEC-11人工审 command execution、production-equivalent timed guard/broker边界、trusted App/nonce/KMS、store trust/time/fence/tag bypass。Covers: B-001–B-031. Owner: verification owner + independent reviewer + human security reviewer. Depends on: T12；GH-699 T3–T6；current-head remote gates。Done when: fresh checks全绿、findings为零，head/CI/0 threads/reviews/merge state一致；merge/release仍按授权。Verify: 记录普通仓库命令与current-head证据。
