...
- [ ] `SP700-T3` 建立 corpus/truth/mapping/protocol/report/summary/review-record schemas、JCS canonicalizer、maintainer-attested reviewer roster verifier和 prior-ledger validator；summary preimage 固定为删除 `summary_digest` 字段后的完整 object，detached attestation不入 preimage；JSON number 仍限 safe integers。Covers: B-004, B-016, B-019, B-022, B-025, B-026, B-027, B-031. Owner: schema/integrity owner. Depends on: SP700-T1与genesis approval。Done when: Rust/Python summary golden一致，self-placeholder/attestation-in-preimage拒绝；distinct fake reviewer IDs with same key/subject拒绝；safe integer/ledger mutation矩阵全绿。Verify: `python3 scripts/ci/validate_public_benchmark.py`; `cargo test --manifest-path vibeguard-runtime/Cargo.toml bench::digest`; `bash tests/test_public_benchmark.sh`.
...
- [ ] `SP700-T10` 接入 strict summary、serialized evidence 与 two-phase publication：summary digest排除自身且 detached attest；attempts/reconciler/publish共用 lease+watermark。phase A 只创建 private attempt draft、上传并重验全部 assets；phase B 先 attest publish_intent再以 draft→published 为唯一 commit point。reconciler 通过 protected environment 只获得精确 draft mutation authority：pre-intent cancel删除对应 draft并记录，post-intent cancel即使 same-tag draft 已存在也只能按匹配 intent 幂等完成/验证；target 与 release-aggregation failure manifest 分别绑定单 target 或完整 failed summary inputs。Covers: B-016, B-018, B-021, B-022, B-027, B-029, B-031. Owner: release/summary/reconciler owner. Depends on: SP700-T2, SP700-T3, SP700-T5, SP700-T7–T9。Done when: summary self-digest mutation、release-wide disagreement/missing-input、partial draft upload、cancel before/after intent/after commit、matching-vs-mismatched draft guard、permission denial、watermark lag与duplicate delivery矩阵各自收敛到唯一合法状态；无 public partial assets。Verify: `bash tests/test_public_benchmark.sh`; `bash tests/test_release_workflow.sh`.
