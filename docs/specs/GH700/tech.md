...
6. valid receipt/nonvalid prepared先 CAS `intent_written`；valid intent 在 publish slot 前绑定 exact human-reviewed new-current base/patch/review plan，nonvalid intent 只绑 unmarked-row plan。pending decurrent须 revoke，merged则 rollback；failed generated/mutation slots用 exact reviewed replacement/compensation或对应 blocked。只有 claim/draft cleanup、revocation、rollback/invalidation或 complete Release+README slot chain可 terminal。
