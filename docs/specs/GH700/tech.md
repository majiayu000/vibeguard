...
3. 首次 Release API/PR mutation前原子 claim，绑定 immutable protected tag identity、plans/liveness/nonce；KMS capsule只供 current exact actor或 expiry后 successor恢复。所有 Release write的 `mutation_kind` exact 为 `{draft_create,draft_update,draft_delete,asset_upload,asset_delete,publish}`；call前 append immutable planned slot（repo/tag/owner/mutation kind/slot/op/predecessor/App、exact pre-state/public request template/expected-post及完整 kind tuple）与 pre-call guard authorization。`draft_create` history只绑定 claim digest/capsule identity/request-template digest；raw nonce由 authenticated secret channel在 broker内存中注入，history、intent、operation ID、日志、报告及 persisted request bytes均不得含 raw或可逆编码；broker只回签 effective-request digest。其它 kind绑定 exact request bytes；fence/lease仅在 envelope。
...
5. protocol surface只绑定 stable `{id,path,grammar,renderer,output schema}`；claim/attempt plan绑定 mutable default ref与逐 surface base/hash/marker/expected/patch，drift须新 slot/nonce/head/render/review。
   valid plan为 rollover、true genesis或 post-invalidation（suffix仅 terminal nonvalid+exact prepared owner）；fresh receipt绑定 current frontier、terminal merged-invalidation digest、owner/all current blobs。mixed/drift或缺 exact receipts blocked。
