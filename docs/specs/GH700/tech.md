...
4. 五种 generated PR在首次 mutation前 planned/bound；response loss全分页查 PR/ref，唯一 active/merged/closed分别 bind/recover/revoke。invalidation plan只含 base/head/reviewed commit/expected tree/blobs/patch/method/ruleset，拒绝 future merge fields；post-merge receipt才绑定 actual merge OID/PR/method/default refs/tree/blobs/owner/fence/frontier/evidence。`post_invalidation_zero` suffix exact closed union另接受验签且 phase-neutral 的三种 trust governance records，不接受 current restoration/其它 owner。
5. protocol surface只绑定 stable `{id,path,grammar,renderer,output schema}`；claim/attempt plan绑定 mutable default ref与逐 surface base/hash/marker/expected/patch，drift须新 slot/nonce/head/render/review。
   valid plan为 rollover、true genesis或 post-invalidation（suffix exact closed union为 terminal nonvalid+exact prepared owner+验签的 phase-neutral trust governance records）；fresh receipt绑定 current frontier、terminal merged-invalidation digest、owner/all current blobs。current restoration/其它 owner/mixed/drift或缺 exact receipts blocked。
...
- valid fixture另覆盖 pre-draft claim/binding、genesis、rollover、pending de-current cancel，以及 draft/五种
  generated PR 的 response-loss/歧义/reject/crash恢复；`post_invalidation_governance_suffix_v1` 接受三种验签 phase-neutral record并逐一拒绝 wrong domain/fence/threshold、phase/liveness/owner mutation、current restoration与 unknown record。
