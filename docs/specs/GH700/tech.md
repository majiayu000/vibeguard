...
publication 使用 attempt-scoped draft 与一个统一 durable state machine；唯一 `publication_authority_sqlite_v1` authority backend、独立 `publication_restore_anchor_dynamodb_v1` anchor、`publication_restore_backup_s3_object_lock_v1` encrypted recovery-byte backend、双 API deployment/bootstrap、lock/fsync/recovery及 canonical secret/history contract只引用 [publication_history_contract.md](publication_history_contract.md)：
1. actors 只按 source/candidate→ledger lease→publication lease→branch CAS；等待 review用 H-006 heartbeat续活。H-006 唯一批准者为 repo release/security maintainer；在 `ttl_seconds`、`heartbeat_period_seconds`、`min_renewal_interval_seconds`、`max_generation_age_seconds` 及 approval/roster digest 全部明确批准并写入 `liveness_policy_digest` 前，publication preflight unavailable 且 claim/heartbeat/takeover 零执行。accepted time只取 canonical contract的 manifest-pinned RFC3161 quorum interval并以 SQLite+DynamoDB high water防回退；renew要求 trusted upper bound早于 prior expiry，takeover要求 trusted lower bound严格晚于 expiry，host/client clock与job absence无效。sequence 1的 `previous_accepted_at=claim_accepted_at`，之后取前一 heartbeat accepted-at；store仅在 prior expiry 前且距 previous accepted time 至少 `min_renewal_interval_seconds` 时续租，`expiry=min(accepted_at+ttl_seconds,claim_accepted_at+max_generation_age_seconds)` 且须严格延长；scheduler 只用 `heartbeat_period_seconds`，generation age cap 后只可 expiry+takeover。product.md 的具体数字仅是未批准 proposal，不是 normative default。
2. frontier唯一为 `(repo_node_id,history_length,history_root,full_prefix_digest)`，history/blocked-ledger leaf framing、root与 full-prefix successor recurrence/goldens 只取 canonical contract；只信 independently anchored digest-pinned trust bundle。append authorization是 closed union：publication records用 owner/publication fence；normal trust rotation/revocation用独立 governance actor/threshold/lease/fence；bootstrap-pinned emergency root cutover在 active key expiry/current threshold loss时用 cold break-glass threshold、restore-grade backup/rollback proof、永久 audit与 privileged higher-epoch anchor，不能绕过 prior anchor或改单调 state。stable rotation approval/ID不含 predecessor/fence；cutover certificate只绑定 exact predecessor、successor ordinal与 next epoch，store以 pre-state或 exact emergency trust append，anchor后才启用 new trust。
   **schemas/publication_history.schema.json** 的唯一 top-level discriminator为 `record_kind`，拒绝 `kind`/`type`/`record_type`及 alias；38-kind exact union、每 kind closed payload field set、四个 closed sub-enums与 non-record durable objects只取 canonical contract的 “Complete `record_kind` union”，tech/tasks不得另列不完整子集。unique indexes、exact predecessor与 actual authorization fence事务验证；ack-loss返原 receipt，fork/ABA拒绝。
   每个 local successor在 repository-global `anchor_commit_gate`先写不含 snapshot/backup/final-request 产物的 `anchor_plan_core` pending row并阻止下一 commit，再把 snapshot/manifest/WAL AEAD-encrypt到独立 Object-Lock exact versions并 strong-read确认，以 detached backup confirmation 固化 final payload/request，取 class-correct quorum并完成 DynamoDB CAS/read-back；本地 confirmation transaction后才返 receipt/放 broker。routine类使用 distinct least-privilege KMS-backed online threshold和短期单次 credential，maintainer keys只用于 privileged类；crash/ack-loss只恢复同一 core/final payload/request/backup/signatures。
3. 首次 Release API/PR mutation前原子 claim；六种 `mutation_kind` exact 为 `{draft_create,draft_update,draft_delete,asset_upload,asset_delete,publish}`。call前 append planned slot，绑定 repo/tag/owner/kind/slot/predecessor/App、exact pre-state/public non-secret request template/expected-post、kind tuple、plan-core/stable slot ID、唯一 `mutation_nonce_digest`与 opaque `mutation_nonce_capsule_id`/`broker_delivery_id`；`draft_create`另绑定 claim digest/capsule。构造先冻结 operation-independent core/slot ID，再算 nonce digest/request commitment，以 slot ID作 capsule AAD，纳入 ciphertext后最后算 payload/op ID，capsule与 plan事务同提交。六种 kind的 raw/reversible mutation nonce及 secret-bearing request/history bytes均 schema-invalid，raw只经 authenticated secret channel；broker重算 derivation、只替换每个 declared secret的唯一 placeholder，回签 `effective_request_digest`/send-once audit并拒绝 same slot/delivery二发；fence/lease仅在 envelope。
   response+postcheck后才 bind completed receipt。任一 uncertainty禁重发，完整发现 Release/assets+broker audit：唯一 post-state→recovered bind；exact pre-state+exhaustive negative+quiescence receipt→not-applied后新 slot；partial/conflict→planned guarded compensation；`draft_create`不可证明只进 `draft_recovery_blocked`，其余五 kind才可进 generic `release_mutation_recovery_blocked`。phase gate要求每 slot∈`{bound,not_applied,compensated}`、intended effect有唯一 bound或已恢复且无 pending/blocked/in-flight/extra；finalization fresh，takeover引用旧 plan fresh authorize。
4. 五种 generated PR在首次 head-ref/commit/PR mutation前 planned，其 closed payload 预绑定 base、head repo/ref、nonce digest、trusted App/installation、reviewed commit/expected tree/patch/method/ruleset 及 replacement chain；response loss全分页查 PR/ref，唯一 active/merged/closed分别 bind/recover/revoke。invalidation plan拒绝 future merge fields；post-merge receipt才绑定 actual merge OID/PR/method/default refs/tree/blobs/owner/fence/frontier/evidence。`post_invalidation_zero` suffix exact closed union另接受以 `publication_terminal_no_publication` 结束的 authenticated same-candidate owner successor chain（takeover=`publication_owner_taken_over`）及四种验签 phase-neutral trust governance records，不接受 nonterminal/forged chain、current restoration/其它 owner。
5. protocol surface只绑定 stable `{id,path,grammar,renderer,output schema}`；claim/attempt plan绑定 mutable default ref与逐 surface base/hash/marker/expected/patch，drift须新 slot/nonce/head/render/review。
   valid plan为 rollover、true genesis或 post-invalidation；true genesis从 manifest-pinned bootstrap root/roster/threshold/first frontier完整重放，允许首次 valid前四种验签 phase-neutral trust governance records（含 anchored emergency root cutover）与 terminal nonvalid/no-publication但拒绝历史 eligible valid/current restoration；post-invalidation suffix为 terminal nonvalid+exact prepared owner+authenticated terminal no-publication chain+同类 governance records。fresh receipt绑定 current frontier、terminal merged-invalidation digest、owner/all current blobs；缺 bootstrap/exhaustive terminal proof、current restoration/其它 owner/mixed/drift或缺 exact receipts blocked。
6. valid 在 CAS `intent_written` 前必须二选一：exact zero receipt，或 rollover `decurrent` planned→bound→merged receipt；后者只在 merge 原子移除旧 current 后可进 intent，failed/retry 先 revoke old gate/PR/head 再 fresh replacement，merged cancellation 走 rollback，禁止双 current。valid intent 还在 publish slot 前绑定 exact human-reviewed new-current base/patch/review plan，nonvalid prepared 直接写只绑 unmarked-row plan 的 intent。failed generated/mutation slots用 exact reviewed replacement/compensation或对应 blocked。只有 claim/draft cleanup、revocation、rollback/invalidation或 complete Release+README slot chain可 terminal。
...
- `blocked_record_kinds_valid_v1` 为八个 exact variants各含一条 schema-complete record；unknown/alias逐条 reject。六个 exact `mutation_secret_boundary_{draft_create,draft_update,draft_delete,asset_upload,asset_delete,publish}_v1`
  goldens只接受 `mutation_nonce_digest`+opaque capsule/delivery+template/effective digests，并逐 kind拒绝 raw/reversible nonce及 secret-bearing request/history bytes；`draft_create` unprovable只接受 `draft_recovery_blocked`且拒绝 generic blocked kind；positive vectors固定 exact JCS bytes/SHA-256。
- valid fixture另覆盖 pre-draft claim/binding、genesis、rollover、pending de-current cancel，以及 draft/五种
  generated PR 的 response-loss/歧义/reject/crash恢复；`rotation_cutover_no_cycle_v1`、`authority_api_trust_v1`、`restore_anchor_rollback_v1`固定无环 ordinal、dual API identity、encrypted Object-Lock backup identity/AEAD/version proof、online-quorum lifecycle、pending-gate serialization及 DynamoDB SigV4/CAS proof；`post_invalidation_suffix_v1` 接受四种验签 phase-neutral record及 exact terminal no-publication/takeover chain，并拒绝 missing/forged/nonterminal/cross-candidate takeover、incomplete negative discovery、wrong governance/break-glass evidence、phase/liveness/owner mutation、current restoration与 unknown record。
