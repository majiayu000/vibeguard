...
1. actors 只按 source/candidate→ledger lease→publication lease→branch CAS；等待 review用 H-006 heartbeat续活。H-006 唯一批准者为 repo release/security maintainer；未批准 publication unavailable。accepted time只取 canonical contract的 manifest-pinned RFC3161 quorum interval并以 SQLite+DynamoDB high water防回退；renew要求 trusted upper bound早于 prior expiry，takeover要求 trusted lower bound严格晚于 expiry，host/client clock与job absence无效。sequence 1的 `previous_accepted_at=claim_accepted_at`，之后取前一 heartbeat accepted-at；store仅在 prior expiry前、间隔≥300s续租，`expiry=min(accepted_at+3600,claim_accepted_at+604800)`且须严格延长；scheduler 900s，7日 cap后只可 expiry+takeover。
