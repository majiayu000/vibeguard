...
- valid fixture另覆盖 pre-draft claim/binding、genesis、rollover、pending de-current cancel，以及 draft/五种
  generated PR 的 response-loss/歧义/reject/crash恢复；`rotation_cutover_no_cycle_v1`、`authority_api_trust_v1`、`restore_anchor_rollback_v1`固定无环 ordinal、同时必需且共享 authority/policy identity 的 HTTPS-mTLS client/Unix-peercred control APIs，以及 DynamoDB table identity/SigV4/CAS proof；`post_invalidation_suffix_v1` 接受三种验签 phase-neutral record及 exact terminal no-publication/takeover chain，并拒绝 missing/forged/nonterminal/cross-candidate takeover、incomplete negative discovery、wrong governance evidence、phase/liveness/owner mutation、current restoration与 unknown record。
