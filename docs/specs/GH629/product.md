...
2. B-002 配置文件不存在时继续使用现有默认值；存在的空对象可按向后兼容规则使用默认值，
   但显式字段必须通过类型、枚举和范围验证。`learn.metrics_tail_bytes` 的 0 当前会退化为
   无界全文件读取，因此 v1 明确要求至少为 1；这不是合法 legacy 值。
...
6. B-006 runtime policy、直接 runtime-config validator/getter 与 setup health check 必须对
   ...
   入口静默接受。Setup 进程状态保持既有 flag precedence：无 machine flag 的 doctor/`--check`
   为兼容 exit 0，bare `--project` 不隐式 strict，`--quiet` 只控制输出，`--no-summary` 保留
   legacy exit 0，`--strict`/`--json`/`--install` 与 verify aliases 按 machine mode 映射；
   不兼容组合继续 exit 64。完整 invocation matrix 以 Tech Spec 为准。
