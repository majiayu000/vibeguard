...
须持 matching source effective epoch 的 shared delivery lease并核对 config digest。ready CAS offset-independent seed；sequencer 分配 offset并原子创建 full reservation + retained allocator claim-binding + tokens，binding 到 registry acknowledgement 后才 GC，防 applied/reclaim crash duplicate。active absent 才重建，仅 matching off 可 freeze；frozen ref digest-bind event timestamp/retention bucket/registry global-lag offset/query scope。pre-barrier route 永久不可达时用预留 token 原子隔离并释放 live slot。closed permanent route 先 fsync per-source quarantine，再 shared root 发布携带同一 query metadata 的 lag stub；receipt delivery 后 quarantine token/locator 仍计 capacity 到 `project_acknowledged`，route 永久失效则 completed ref 原子转入该 token 的 quarantine。rebind 重取 completed capacity；rebound permanent failure 用同 token/slot 的 reserved scratch generation COW handoff，one-entry-full 不需第二 entry/token。
worker 只 fsync/create keyed slot、提交 global applied/reclaim，不写 project journal；source coordinator 按 shared delivery lease → project lock 持有 epoch 到 `projection_done` fsync；global success 唯一为后续 global CAS `project_acknowledged`。
ack 后先 CAS stub 为 digest-bound `retirement_pending`，再 retire/reclaim source entry，最后验证并删除 stub/释放 token；任一 crash 向前恢复。未 ack completed ref/recovery locator 必须 pin/计 backpressure或移入可枚举 lag index，retention GC 不得删除；只有 rebind+ack retirement 或 source-lock terminal discard proof 才能 release。off drain 把 keyed receipt 原子 handoff 到 per-source admin ref并释放 shared completed token；effective off 要求 shared refs 为零，新 off request 须原子 adopt/retag 旧 admin set。
...
H-001–H-020 已批准。当前共 146 条唯一 repo paths：105 条 existing、41 条 planned。
...
| U-22 measured coverage | planned **scripts/ci/self-application/u22-critical-files.json**; `scripts/ci/self-application/check-u22-coverage.sh`; tests/manifests/Cargo/CI | runtime/sidecar each ≥80%；100% line+branch: runtime `{git_root,project_config,codex_app_server_core,codex_app_server_file_changes,codex_app_server_hooks,codex_app_server_strategies,hook_orchestrator_context,hook_orchestrator,hook_orchestrator_post_edit,hook_orchestrator_post_edit_history,hook_orchestrator_learn,hook_checks,hook_checks_history,log_query,event_schema}.rs`、`observe/{aggregate,prometheus,read,render,stats_summary}.rs`、`semantic_defense/{mod,config,identity,protocol,provider,inventory,inventory_adapters/mod,inventory_adapters/typescript_npm,test_weakening,runtime_signal,cache,metrics}.rs`、sidecar `{protocol,sandbox}.rs`；每个 critical file 恰一次携带非空 exact `owner_suites`，gate 双向核对 Product-to-Test mapping/Cargo exact name/shell selector，missing/empty/unknown/duplicate/zero-match/rename-drift/无反向 owner、condition arms 与 aggregate masking 均失败；independent B-001/B-003/B-009/B-011–B-015/B-017–B-020/B-022–B-028/B-035/B-037 matrices map every branch ID |
...
| B-002 flag-off parity | hook orchestration | `bash tests/hooks/test_semantic_defense.sh flag_off_parity`、`bash tests/hooks/test_runtime_rule_signals.sh disable_freezes_pending_backlog`、`cross_project_receipt_freeze`、`direct_opt_out_acknowledgement`、`source_off_live_capacity_drain`、`source_off_completed_capacity_handoff`、`off_terminal_proof`、`off_request_admin_adoption`、`frozen_lag_query_scope`、`off_preparing_config_reversal`、`off_preparing_new_off_restart`；effective commit requires zero shared completed refs；no unack age-delete；enable/new digest preserves admin ownership；no leak |
...
| B-035 truthful rendering | doctor/status/observe/stats/health/readers | B-020 consumer commands plus `bash tests/test_setup_check.sh semantic_projection_rendering`、`bash tests/test_hook_status.sh semantic_projection_rendering`、`bash tests/hooks/test_semantic_defense.sh status_rendering_and_redaction`、`bash tests/test_observe.sh semantic_barrier_projection`、`bash tests/test_stats.sh semantic_barrier_projection`；`cargo test --manifest-path vibeguard-runtime/Cargo.toml hook_orchestrator_learn::tests::recent_log_error_is_fail_visible -- --exact`、`hook_orchestrator_learn::tests::metrics_error_is_fail_visible`、`hook_orchestrator_learn::tests::projection_lag_has_zero_suggestion`；project-local barrier/marker vs global-only `project_acknowledged`；Learn errors typed/zero candidate；v1 legacy；mixed lag empty；all formats agree |
...
completed host event + project/session/change
  ...
  → project all_activated barrier (ordered stage/activation receipt digests)
       ...
       └─ any global worker verifies barrier + eligibility epoch
              ...
              → source coordinator lock → projection_done → global project_acknowledged or visible lag
```
