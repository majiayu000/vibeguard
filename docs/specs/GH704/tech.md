...
须持 matching source effective epoch 的 shared delivery lease并核对 config digest。ready 先在 registry CAS `claim_prepared`，释放 registry 后才创建 exact reservation，再 CAS/reclaim；crash/off 先核对 claim+reservation，不得 reserve-before-claim。requested off 用 writer-fair exclusive lease 持久化 off-preparing，禁止新 L2，bounded drain registry/claim/reservation/outbox 到 fsynced keyed slots 并 reclaim global capacity，全部为零才 effective；否则 pending/error。receipt
...
- event log、precision 与 Learn 只消费上述 canonical project typed event。doctor 的
  ...
  `oneOf` 保留 exact closed v1，v2 必需 `semantic_projection {state,finalized,barrier_refs,barrier_set_digest,projection_watermark,lag_refs}`。
  bounded sorted refs 逐 event 绑 source/event/barrier/receipt/offset，set digest 绑 query + ordered barrier/lag refs + registry/allocator/outbox generations/tail，watermark 携带同一 snapshot；scan 前后 generations 必须不变，drift 只 bounded retry 或 lag/unavailable + empty；mixed lag/proof-overflow 全部 empty。
  current writer 只发 v2；v1 reader 映射 legacy-untracked/empty。Codex status、quality grader、constraint-frequency 与 Rust hook/log enforcement-history readers 都 join barrier，pending/aborted/lag 不得成为 success/grade/rule hit/history decision。
...
H-001–H-020 已批准。当前共 143 条唯一 repo paths：102 条 existing、41 条 planned。
