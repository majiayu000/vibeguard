...
- W-02 reducer 只消费同一 stable hypothesis 的 ordered attempts，每次 attempt 必须有
  ...
  candidate context。
- W-02 的 hypothesis/attempt/fresh-failure/reset/retention-tombstone tuples、edge transitions、selectors、
  crash/illegal-transition evidence 与 L1 publish 偏序只从
  [integrity_retention_model.json](integrity_retention_model.json) 读取；schema 为
  [integrity_retention_model.schema.json](integrity_retention_model.schema.json)，stdlib verifier 为
  [verify_integrity_retention_model.py](verify_integrity_retention_model.py)。policy epoch 输入是 finite、closed、
  digest-bound source/project membership；verifier 必须重算 deterministic bijective expansion，逐 relation
  展开 edge_id×tuple_id×selector_id×transition_id 的所有 step crash 与非法 from-state。
...
ResourceLedger 仍只拥有 capacity/ownership；W-02 evidence identity/retention 不得在 Markdown 或 capacity
model 中维护第二份 tuple/edge/fault 表，且 W-02 L1 publish 还必须通过 integrity-retention model 的完整偏序。
...
receipt worker 只 fsync/create keyed slot、提交 global applied/reclaim/`receipt_delivered` 并保留 token；每个 slot 在 offset 前预留独立 per-source entry/byte entitlement，ack/terminal handoff 后按 global pending→exact unlink+directory fsync/retired proof→global release 三阶段回收。source coordinator 按 shared delivery lease → project lock fsync `projection_done`，global CAS `project_acknowledged` 后才释放 live token与未消费 allocator admin entitlement；ack 前 permanent/retention 原子转 `quarantine_ack_pending`。A/B replacement 走 inactive stage→stub repoint→old reclaim，one-entry-full 不需第二 token。
retirement 走 global pending→source retired proof→global release，并在最终 CAS 释放 consumed allocator admin entitlement；root receipt 处理 lost ack 且禁止双释。未 ack locator 必须 pin/计 backpressure或原子移入可枚举 lag index，retention GC 不得 age-delete；只有 rebind+ack retirement 或 source-lock terminal discard proof 才可 release。deleted-root 特例使用 broker-owned identity-bearing object handle/capability；old-path ENOENT 或 rename/move 只进入 relocated rebind，只有 verified object delete-complete 才 terminalize。off drain 将 durable keyed receipt 直接 handoff 到 query-scoped per-source admin ref并释放 shared token；completed `receipt_delivered`/pending 是 off-blocking 且 handoff 前 capacity-bearing，retained `project_acknowledged` history 不阻塞；新 request 以 adopt-all 或 per-ref terminal-discard-all 二选一闭合旧 set，present/absent config identity 均须 stable parent-capability no-follow 重验。
...
- event log、precision 与 Learn 只消费上述 canonical project typed event。project-local canonical
  ...
  bounded sorted refs 逐 event 绑 source/event/barrier/receipt/offset，set digest 绑 query + ordered barrier/lag refs + global root 与 `registry_generation/allocator_generation/outbox_generation/completed_index_generation/admin_index_generation/success_history_generation` + allocator tail，watermark 携带同一 snapshot；scan 前后六个 generations 必须全部不变，drift 只 bounded retry 或 lag/unavailable + empty；mixed lag/proof-overflow 全部 empty。
...
| ResourceLedger + canonical project journal | `vibeguard-runtime/src/event_schema.rs`; `vibeguard-runtime/src/{hook_orchestrator,hook_orchestrator_post_edit,hook_checks_common,log_append}.rs`; `hooks/_lib/log_write.sh`; `scripts/gc/gc-logs.sh`; planned **vibeguard-runtime/src/semantic_defense/runtime_signal.rs**; `schemas/event-log.schema.json`; planned **tests/hooks/test_runtime_rule_signals.sh**; `tests/test_gc_logs_rotation.sh`; `tests/test_gc_logs_concurrent.sh`; `tests/test_observability_schemas.sh`; `docs/specs/GH704/runtime-integrity.md` | one closed resource enum/reducer and one owner per kind；bundle terminal totality + replay receipts；project/derived/journal common no-early-credit compaction；allocator metadata fixed A/B alias；history GC and adoption preflight；all Rust/shell writers share tail-read→WAL fsync→journal append lease；GC honors pins/watermarks |
