...
须持 matching source effective epoch 的 shared delivery lease并核对 config digest。ready 先在 registry CAS `claim_prepared`，释放 registry 后才创建 exact reservation + completed-index token 的 atomic root generation，再 CAS/reclaim；active recovery 缺 reservation 时幂等重建，仅 matching off-preparing 可 freeze，不得 reserve-before-claim。requested off 用 writer-fair exclusive lease 持久化 off-preparing，禁止新 L2，bounded drain registry/claim/reservation/outbox 到 fsynced keyed slots + completed index 并 reclaim global capacity，全部为零且 config identity/digest 仍匹配才 effective；enable 反转则 rebind，另一 off request 重启 epoch；否则 pending/error。receipt
