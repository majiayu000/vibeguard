...
须持 matching source effective epoch 的 shared delivery lease并核对 config digest。ready CAS offset-independent seed；sequencer 分配 offset并原子创建 full reservation + retained allocator claim-binding + tokens，binding 到 registry acknowledgement 后才 GC，防 applied/reclaim crash duplicate。active absent 才重建，仅 matching off 可 freeze；frozen ref digest-bind event timestamp/retention bucket/registry global-lag offset/query scope。closed permanent route 先 fsync per-source quarantine，再 shared root 发布 lag stub；receipt delivery 后 quarantine token/locator 仍计 capacity 到 `project_acknowledged`，route 永久失效则 completed ref 原子转入该 token 的 quarantine。rebind 重取 completed capacity；rebound permanent failure 用同 token/slot 的 reserved scratch generation COW handoff，one-entry-full 不需第二 entry/token。
...
ack 后先 CAS stub 为 digest-bound `retirement_pending`，再 retire/reclaim source entry，最后验证并删除 stub/释放 token；任一 crash 向前恢复。未 ack completed ref/recovery locator 必须 pin/计 backpressure 或原子移入可枚举 lag index，retention GC 不得删除。off drain 直接将 durable keyed receipt 原子 handoff 到 per-source admin/quarantine ref并释放 shared completed token；effective off 要求该 source shared completed refs 为零，admin ref 仅可 bounded retention-terminal cleanup 或 re-enable capacity reacquire/rebind。
...
| B-002 flag-off parity | hook orchestration | `bash tests/hooks/test_semantic_defense.sh flag_off_parity`、`bash tests/hooks/test_runtime_rule_signals.sh disable_freezes_pending_backlog`、`cross_project_receipt_freeze`、`direct_opt_out_acknowledgement`、`source_off_live_capacity_drain`、`source_off_completed_capacity_handoff`、`off_terminal_cleanup`、`frozen_lag_query_scope`、`off_preparing_config_reversal`、`off_preparing_new_off_restart`；effective commit requires zero shared completed refs；bounded admin cleanup/rebind；enable/new digest resets epoch+cursor；no leak |
