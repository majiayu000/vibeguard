...
ack 后先 CAS stub 为 digest-bound `retirement_pending`，再 retire/reclaim source entry，最后验证并删除 stub/释放 token；任一 crash 向前恢复。未 ack completed ref/recovery locator 必须 pin/计 backpressure 或原子移入可枚举 lag index，retention GC 不得删除。
