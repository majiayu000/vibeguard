...
须持 matching source effective epoch 的 shared delivery lease并核对 config digest。ready CAS offset-independent seed；sequencer 分配 offset并原子创建 full reservation + retained allocator claim-binding + completed/quarantine tokens；每个 quarantine token 初始即预留 closed-max A/B replacement generations。binding 到 registry acknowledgement 后才 GC，防 applied/reclaim crash duplicate。active absent 才重建，仅 matching off 可 freeze + global lag ref。off drain 后复核 config。closed permanent delete/replace/stable ACL-denied 先 fsync independent per-source quarantine，再 shared root 发布 lag stub、释放 outbox/completed token；source corruption 不阻塞 global，rebind 重取 completed capacity。receipt
worker 只 fsync/create keyed slot、提交 global applied/reclaim/`receipt_delivered` 并保留 quarantine token，不写 project journal；source
coordinator 按 shared delivery lease → project lock 持有 epoch 到 `projection_done` fsync；global CAS `project_acknowledged` 后才释放 token；ack 前 permanent/retention 用同 token 原子转 `quarantine_ack_pending`。replacement 走 A/B stage→stub repoint→old reclaim；retirement 走 global pending→source retired proof→global release。off 用
...
- event log、precision 与 Learn 只消费上述 canonical project typed event。project-local canonical
  ...
  current writer 只发 v2；v1 reader 映射 legacy-untracked/empty。Codex status、quality grader、constraint-frequency 与 Rust hook/log enforcement-history readers 按读取边界 join barrier + local `projection_done` 或 global `project_acknowledged`；pending/aborted/frozen/quarantined/lag 不得成为 success/grade/rule hit/history decision。
