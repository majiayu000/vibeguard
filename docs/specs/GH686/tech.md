...
当前执行等级是**人工自证（manual attestation），不是自动化门禁**：CI 只断言 PR
模板存在该证据段并运行本门的确定性单测，不会对规则 diff 实际运行配对评测，也不会
交叉核验模板里填写的 delta/模型 ID/digest 与所附报告是否一致。报告数字的真实性由
reviewer 对照所附 report.json 人工核对把关。机器化核验（解析 PR 描述并与 artifact
报告比对）是后续工作，不在本 spec 范围。
