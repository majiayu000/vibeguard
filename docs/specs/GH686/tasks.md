...
- [ ] `SP686-T1` 在 `eval/` 下新增 `run_paired_eval.py`，实现候选规则的双来源剔除与四条断言。Covers: B-001, B-002, B-003, B-012. Owner: implementation agent. Done when: 规则树按小节剔除、core 文件按表格行剔除；逐文件差分（仅一个文件不同且差异恰为候选小节，不在拼装文本上重放剔除）、在场断言（候选不存在则终止）、定义计数断言（复用独立 canonical `RULE_ID_HEADING_RE`，覆盖含三个 TASTE ID 在内的 127 条定义；差恰为 1 且删除 span 内无第二个 `^## `，专抓删多了）、定义位点 token 断言四条齐备；断言范围限定在定义位点而非全文，交叉引用残留逐条列出且超 `max_cross_refs` 判 inconclusive；整文件删除、no-op 剔除、贪婪多删一节均被拒；候选位于文件末节时必须能跑通；任一断言失败非零退出。Verify: `bash tests/test_paired_eval.sh`。
- [ ] `SP686-T2` 目标 / 非目标样本划分与新建 non-target 数据集。Covers: B-004, B-015. Owner: implementation agent. Done when: 目标样本用精确 `==` 匹配、不复用 `filter_samples`（它是前缀匹配且强制混入 `NONE`）；non-target 数据集独立于 `v1.jsonl`、使用普通任务输入与质量 rubric，而非复用 tp/fp schema；两组皆空时非零退出。Verify: `bash tests/test_paired_eval.sh`。
- [ ] `SP686-T3` 四次 producer 运行、pairwise judge、指标计算、合取判定与退出码语义。Covers: B-005, B-007, B-011, B-015. Owner: implementation agent. Done when: 目标轴复用 structured-JSON grader；非目标轴不复用 code-review JSON system prompt，而是用同一 producer 模型与规则文本生成普通任务响应，再对每个样本盲化 A/B 并换序调用 judge 两次；真实运行显式要求 `--judge-model`，严格解析 `winner ∈ {A,B,tie}`，judge 失败计入 skipped，映射结论不一致时轴为 `inconclusive`；保存 producer/judge model ID、judge prompt digest、两次原始判定和映射结果；不复用 `model_summary_metrics`（它把跳过样本剔出分母）；分母为请求样本数；跳过率与跳过率差超阈值判 `inconclusive`；目标轴用严格大于；单轴通过不得整体通过。真实运行的 `inconclusive` 与不通过均非零退出。Verify: `python3 eval/test_paired_eval.py`。
...
- [ ] `SP686-T8` 全规则遍历 smoke。Covers: B-003, B-012. Owner: implementation agent. Done when: 对全仓 127 条规则 ID（含三个 TASTE ID）各跑一次 dry-run（无模型调用，秒级），全部跑完；位于文件末节的 18 条规则（含 `rules/claude-rules/common/evidence-provenance.md` 的 W-21、`rules/claude-rules/common/no-silent-degradation.md` 的 U-29 与 `rules/claude-rules/rust/quality.md` 的 TASTE-PANIC-MSG）不越界；10 个单规则文件剔除后的空壳残留出现在报告中；被交叉引用的 19 条规则跑完并列出残留而非崩溃。Verify: `bash tests/test_paired_eval.sh`。
- [ ] `SP686-T9` 计数正则的校准锚点与文件集合断言。Covers: B-003. Owner: implementation agent. Done when: 使用现有独立 canonical `scripts/lib/vibeguard_manifest.py:35` 的 `RULE_ID_HEADING_RE`；常数锚点测试断言它在 `rules/claude-rules/**` 上的匹配数等于当前定义总数（本会话实测 **127**）、含三个 TASTE ID且无重复 ID；`set(real_files) == set(tmp_files)` 是独立于内容比较的先行断言。Verify: `python3 eval/test_paired_eval.py`。理由：计数断言是"删多了"方向的唯一非同源兜底，正则漏匹配会让它静默失真而不是报错。
- [ ] `SP686-T10` `max_cross_refs` 边界的可见性断言。Covers: B-012. Owner: implementation agent. Done when: 显式断言 U-32 的跨节残留为 4 处（`rules/claude-rules/common/security.md:121`、`:178`、`:202`，`rules/claude-rules/common/coding-style.md:164`；同文件 `:115` 在 U-32 自身小节内，随剔除一并移除，不计入），且按临时 `max_cross_refs: 4` 位于允许边界、不因交叉引用单独变成 `inconclusive`；再以 5 处 fixture 断言超过边界时为 `inconclusive`。该阈值仍受 `calibrated: false` 总门约束，不得据此输出 pass。Verify: `python3 eval/test_paired_eval.py`。
