...
- [x] `SP686-T11` 安慰剂对照与长度混杂的可见性。Covers: B-013, B-014. Owner: implementation agent. Done when: 报告输出 with/without 规则文本字符数与差值，与两轴 delta 并列；标定流程只接受维护者明确审核为语义无关、不同于候选且按完整 `load_rules` prompt 删除差（含 core 行）计算后长度差比例不超过 `max_placebo_length_ratio` 的 placebo pair；未映射组合、匿名 compact 等价语义或交叉引用残留超过 `max_cross_refs` 时在付费调用前拒绝；placebo 为每个目标样本重新运行基线/删除 arm，交替首发并记录调度。Verify: `bash tests/test_paired_eval.sh`。
