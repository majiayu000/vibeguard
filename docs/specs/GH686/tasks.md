...
- [ ] `SP686-T10` `max_cross_refs` 边界的可见性断言。Covers: B-012. Owner: implementation agent. Done when: 显式断言 U-32 按当前 `main` 有 13 处候选小节外引用，超过 `max_cross_refs: 4` 时判为 `inconclusive`；另以 4 处与 5 处 fixture 分别断言允许边界和超界行为。默认阈值不因规则树内容增长而放宽，且仍受 `calibrated: false` 总门约束，不得据此输出 pass。Verify: `python3 eval/test_paired_eval.py`。
