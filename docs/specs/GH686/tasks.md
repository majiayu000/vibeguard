...
- [x] `SP686-T2` 目标 / 非目标样本划分与新建 non-target 数据集。Covers: B-004, B-015. Owner: implementation agent. Done when: 目标样本用精确 `==` 匹配、不复用 `filter_samples`（它是前缀匹配且强制混入 `NONE`）；non-target 数据集独立于 `v1.jsonl`、使用普通任务输入与质量 rubric，而非复用 tp/fp schema；两组皆空时非零退出。Verify: `bash tests/test_paired_eval.sh`。
...
- [x] `SP686-T5` 在 `eval/paired/` 下新增 `thresholds.json` 与未标定强制降级。Covers: B-006, B-010. Owner: implementation agent. Done when: 任一轴样本量不足报 `inconclusive`；`calibrated: false` 时整体强制 `inconclusive`、永不输出 `pass`。Verify: `python3 eval/test_paired_eval.py`。
...
- [x] `SP686-T11` 安慰剂对照与长度混杂的可见性。Covers: B-013, B-014. Owner: implementation agent. Done when: 报告输出 with/without 规则文本字符数与差值，与两轴 delta 并列；标定流程含 placebo 运行。Verify: `bash tests/test_paired_eval.sh`。
