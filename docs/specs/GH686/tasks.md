...
- [x] `SP686-T1` 在 `eval/` 下新增 `run_paired_eval.py`，实现候选规则的双来源剔除与四条断言。Covers: B-001, B-002, B-003, B-012. Owner: implementation agent. Done when: 规则树按小节剔除、core 文件按表格行剔除；小节边界是下一条 canonical 规则定义而非任意 Markdown `##`；逐文件差分（仅一个文件不同且差异恰为候选小节，不在拼装文本上重放剔除）、在场断言（候选不存在则终止）、定义计数断言（复用独立 canonical `RULE_ID_HEADING_RE`，覆盖含三个 TASTE ID 在内的 127 条定义；差恰为 1 且删除 span 内无第二条 canonical 规则定义，专抓删多了）、定义位点 token 断言四条齐备；断言范围限定在定义位点而非全文，交叉引用残留逐条列出且超 `max_cross_refs` 判 inconclusive；整文件删除、no-op 剔除、贪婪多删一节均被拒；候选位于文件末节和正文含普通 `##` 标题时必须能跑通；任一断言失败非零退出。Verify: `bash tests/test_paired_eval.sh`。
- [x] `SP686-T2` 目标 / 非目标样本划分与新建 non-target 数据集。Covers: B-004, B-015. Owner: implementation agent. Done when: 目标样本用精确 `==` 匹配、不复用 `filter_samples`（它是前缀匹配且强制混入 `NONE`）；non-target 数据集独立于 `v1.jsonl`、使用普通任务输入、质量 rubric 与候选 `excluded_rules`，而非复用 tp/fp schema；候选相关样本从该次非目标轴排除；两组皆空时非零退出。Verify: `bash tests/test_paired_eval.sh`。
...
- [x] `SP686-T5` 在 `eval/paired/` 下新增 `thresholds.json` 与未标定强制降级。Covers: B-006, B-010. Owner: implementation agent. Done when: 任一轴样本量不足报 `inconclusive`；比例阈值必须是 0–1 的有限数，计数阈值必须是非负整数；`calibrated: false` 时整体强制 `inconclusive`、永不输出 `pass`。Verify: `python3 eval/test_paired_eval.py`。
...
- [x] `SP686-T11` 安慰剂对照与长度混杂的可见性。Covers: B-013, B-014. Owner: implementation agent. Done when: 报告输出 with/without 规则文本字符数与差值，与两轴 delta 并列；标定流程含不同于候选且长度差比例不超过 `max_placebo_length_ratio` 的 placebo 运行。Verify: `bash tests/test_paired_eval.sh`。
