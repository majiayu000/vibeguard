...
- [x] `SP686-T1` 在 `eval/` 下新增公共入口 `run_paired_eval.py` 与实现模块 `paired_runner.py`，实现候选规则的双来源剔除与四条断言。Covers: B-001, B-002, B-003, B-012. Owner: implementation agent. Done when: 规则树按小节剔除、core 文件按表格行剔除；小节边界是下一条 canonical 规则定义而非任意 Markdown `##`；逐文件差分（仅一个文件不同且差异恰为候选小节，不在拼装文本上重放剔除）、在场断言（候选不存在则终止）、定义计数断言（复用独立 canonical `RULE_ID_HEADING_RE`，覆盖含三个 TASTE ID 在内的 127 条定义；差恰为 1 且删除 span 内无第二条 canonical 规则定义，专抓删多了）、定义位点 token 断言四条齐备；断言范围限定在定义位点而非全文，交叉引用残留逐条列出且超 `max_cross_refs` 判 inconclusive；匿名 compact L1-L7 仍保留等价语义的已知候选在付费调用前明确拒绝；整文件删除、no-op 剔除、贪婪多删一节均被拒；候选位于文件末节和正文含普通 `##` 标题时必须能跑通；任一断言失败非零退出。Verify: `bash tests/test_paired_eval.sh`。
- [x] `SP686-T2` 目标 / 非目标样本划分与新建 non-target 数据集。Covers: B-004, B-015. Owner: implementation agent. Done when: 目标样本用精确 `==` 匹配、不复用 `filter_samples`（它是前缀匹配且强制混入 `NONE`）；non-target 数据集独立于 `v1.jsonl`、使用普通任务输入、质量 rubric 与候选 `excluded_rules`，而非复用 tp/fp schema；候选相关样本从该次非目标轴排除，排除 ID 必须存在于当前规则树 canonical inventory，目标与非目标样本对象重复键必须拒绝；两组皆空时非零退出。Verify: `bash tests/test_paired_eval.sh`。
- [x] `SP686-T3` 四次 producer 运行、pairwise judge、指标计算、合取判定与退出码语义。Covers: B-002, B-005, B-007, B-011, B-015. Owner: implementation agent. Done when: 目标轴复用 structured-JSON grader 且 scorer 的 malformed JSON/重复键按 skipped 处理；非目标轴不复用 code-review JSON system prompt，而是用同一 producer 模型与规则文本生成普通任务响应，再对每个样本盲化 A/B 并换序调用 judge 两次；producer 空白响应按 skipped 处理，不得送入 judge；真实运行显式要求 `--judge-model`，严格解析 `winner ∈ {A,B,tie}`，judge 失败、malformed JSON 或重复键计入 skipped 且付费 raw 仍保留，映射结论不一致时轴为 `inconclusive`；保存 producer/judge model ID、judge prompt digest、两次原始判定和映射结果；入口导入仓库内评估器模块前固定 commit，规则树/core/数据集/阈值/模型基线/由 import 关系推导的评估器本地依赖闭包在输入准备前后都必须由该提交跟踪且干净，规则 Markdown 集合必须与固定提交一致，符号链接输入必须拒绝，并逐文件对比实际 worktree blob 与固定 commit blob，Git 隐藏标记不得绕过；首个模型调用前预留可写报告路径，commit 无法解析、输入 dirty/untracked/仓外或目标不可写时零客户端创建/零调用失败；不复用 `model_summary_metrics`（它把跳过样本剔出分母）；分母为请求样本数；跳过率与跳过率差超阈值判 `inconclusive`；Ctrl-C 停止新请求、保留已付费响应并将未完成项记为 skipped，写出带 interruption stage 的 partial report；目标轴用严格大于；单轴通过不得整体通过。真实运行的 `inconclusive` 与不通过均非零退出。Verify: `bash tests/test_paired_eval.sh`。
- [x] `SP686-T4` 离线 dry-run 路径。Covers: B-008. Owner: implementation agent. Done when: 无 API 密钥可运行，输出四次运行的输入身份、四条断言结果、交叉引用与空壳文件残留清单、样本划分及选中/排除样本 ID；**dry-run 不产出判定，正常完成退出 0**，不受 B-010/B-011 约束。Verify: `bash tests/test_paired_eval.sh`。
- [x] `SP686-T5` 在 `eval/paired/` 下新增 `thresholds.json` 与未标定强制降级。Covers: B-006, B-010. Owner: implementation agent. Done when: 任一轴样本量不足或为空报 `inconclusive`；两个最小样本阈值必须是正整数，`max_cross_refs` 必须是非负整数，比例阈值必须是 0–1 的有限数，重复 JSON 键必须拒绝；`calibrated: false` 时整体强制 `inconclusive`、永不输出 `pass`。Verify: `bash tests/test_paired_eval.sh`。
...
- [x] `SP686-T10` `max_cross_refs` 边界的可见性断言。Covers: B-012. Owner: implementation agent. Done when: 规则树候选小节与 core 候选表格行之外的显式引用均进入污染计数；显式断言 U-32 按当前 `main` 有 13 处残留引用，超过 `max_cross_refs: 4` 时判为 `inconclusive`；另以 4 处与 5 处 fixture 分别断言允许边界和超界行为。默认阈值不因规则树内容增长而放宽，且仍受 `calibrated: false` 总门约束，不得据此输出 pass。Verify: `python3 eval/test_paired_eval.py`。
- [x] `SP686-T11` 安慰剂对照与长度混杂的可见性。Covers: B-013, B-014. Owner: implementation agent. Done when: 报告输出 with/without 规则文本字符数与差值，与两轴 delta 并列；标定流程只接受维护者明确审核为语义无关、不同于候选、有至少一个真实目标样本且按完整 `load_rules` prompt 删除差（含 core 行）计算后长度差比例不超过 `max_placebo_length_ratio` 的 placebo pair；未映射组合、零目标样本、匿名 compact 等价语义或交叉引用残留超过 `max_cross_refs` 时在付费调用前拒绝；placebo 为每个目标样本重新运行基线/删除 arm，交替首发并记录调度。当前审核 pair 为有现成目标样本且删除长度相近的 `GO-01 / SEC-07`。Verify: `bash tests/test_paired_eval.sh`。
...
T1 / T4 与 T3 / T5 都写配对评估入口或实现模块，因此**不拆并行写 lane**，由单 agent
