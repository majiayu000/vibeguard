...
- [ ] `SP686-T1` 在 `eval/` 下新增 `run_paired_eval.py`，实现输入构造与身份断言。Covers: B-001, B-002, B-003. Owner: implementation agent. Done when: with/without 两次运行共用模型 ID、数据集摘要、样本集摘要；候选规则在场/缺席有机械断言；任一不满足则非零退出。Verify: `bash tests/test_paired_eval.sh`。
...
- [ ] `SP686-T5` 新增 `eval/paired/` 阈值文件 `thresholds.json` 与未标定警告。Covers: B-006. Owner: implementation agent. Blocked by: SP686-D3. Done when: 样本量不足报 `inconclusive`；未标定阈值运行时有显式警告。Verify: `python3 eval/test_paired_eval.py`。
