...
| B-005 offline PR decision 与条件式 merge evidence | `checks/pr_gate.py`、PR fixtures | `bash tests/test_specrail_adoption.sh` 证明 `pr-clean-authorized` allowed，missing-auth/pending-CI/unresolved-thread 均 non-allowed（`blocked` 或 `needs_human`）；另对 `pr-merge-confirmed`、`pr-merge-missing-path`、`pr-merge-unconfirmed-local-failure`、`pr-query-after-merge` 运行 `checks/pr_gate.py`，验证声明 dispatch/record 后的路径、remote confirmation、SHA 与时间顺序 |
