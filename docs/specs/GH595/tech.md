...
| GitHub PR evidence adapter | `checks/github_pr_evidence.py`、`checks/github_review_threads.py` | 只读收集 PR/issue evidence，按唯一 ID 分页收集 review threads，并在采集前后比较 head 与 issue relation | 阻止 stale、不完整或混合快照冒充 current evidence |
...
`github_pr_evidence.py` 先读取 PR snapshot，再按非空唯一 ID 分页读取 GraphQL
review threads 到 `totalCount`，随后再次读取 PR snapshot。ID 缺失/重复、distinct
count 不匹配、分页游标不前进、连接仍有下一页、节点数与 `totalCount` 不一致，
或两次 `headRefOid` / issue relation 不一致时直接失败；
...
| B-004 current-head 只读 evidence 与 drift detection | `checks/github_pr_evidence.py`、`checks/github_review_threads.py`、`checks/github_issue_reference.py` | `bash tests/test_specrail_adoption.sh` 覆盖 review-thread 全分页、唯一 ID、不完整 connection fail closed 和 double-read evidence |
