...
| B-003 workflow/all-spec validation 覆盖 configured root | `checks/check_workflow.py`、`.github/workflows/workflow-check.yml` | `python3 checks/check_workflow.py --repo . --spec-dir docs/specs/GH595`; `python3 checks/check_workflow.py --repo . --all-specs`；smoke 拒绝 missing/non-directory root |
