...
| Offline PR gate | `checks/pr_gate.py:457-560` | 综合 PR 状态、head、CI、review、threads、review source、merge state、merge record 和授权，返回稳定 decision | 将确定性缺口与仅缺人工授权区分为 `blocked` / `needs_human` |
...
`github_pr_evidence.py` 先读取 PR snapshot，再读取 GraphQL review threads，随后
再次读取 PR snapshot。两次 `headRefOid` 或 issue relation 不一致时直接失败；
...
review，除非它是已报告 reviewer-lane failure 的显式授权恢复路径；merge record
还必须包含可验证的 remote confirmation。
...
VibeGuard overrides 未漂移、matrix 中所有 target-local artifact 存在、错误
`product_spec=README.md` 不会冒充 configured GH packet，并对 PR gate/runtime
ledger 执行 allowed 与 fail-closed fixtures。独立 workflow 在 PR 和 main push 上
运行 `--all-specs`、adoption smoke 与 `git diff --check`。
...
| --- | --- | --- |
| B-001 required adoption assets 完整且非法资产 fail closed | pack files、`checks/check_workflow.py`、schemas、fixtures | `python3 checks/check_workflow.py --repo .`; `bash tests/test_specrail_adoption.sh` |
| B-002 VibeGuard configured paths/locale/auth mode 保持 | `workflow.yaml`、`checks/specrail_lib.py`、configured-artifact negative smoke | `bash tests/test_specrail_adoption.sh`；确认 `README.md` 不能满足 GH539 configured product path |
| B-003 workflow/all-spec validation 覆盖 configured root | `checks/check_workflow.py`、`.github/workflows/workflow-check.yml` | `python3 checks/check_workflow.py --repo . --spec-dir docs/specs/GH595`; `python3 checks/check_workflow.py --repo . --all-specs` |
| B-004 current-head 只读 evidence 与 drift detection | `checks/github_pr_evidence.py`、`checks/github_issue_reference.py` | `python3 -m pytest -q`; `python3 checks/github_pr_evidence.py --help`；人工审查 double-read 分支 |
| B-005 offline PR decision 分离 deterministic/human gap | `checks/pr_gate.py`、PR fixtures | `bash tests/test_specrail_adoption.sh`；`pr-clean-authorized` allowed，missing-auth `needs_human`，pending-CI/unresolved-thread blocked |
...
| B-010 pinned provenance 与 local/external evidence 分离 | `AGENT_USAGE.md`、`docs/ADOPTION_MATRIX.md`、`examples/adoptions/matrix.json` | `bash tests/test_specrail_adoption.sh`；确认 pin 为 `e9c82f55b11826cffab9f7c95cd3f3413428ee45` |
