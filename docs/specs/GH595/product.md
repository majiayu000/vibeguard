...
4. B-004 GitHub PR evidence 收集必须只读，按非空唯一 thread ID 完整读取 review
   threads 的全部分页，并在读取前后两次确认 PR head 与 issue relation 未漂移；
   ID 重复/缺失、distinct count 与 `totalCount` 不符、分页不完整、漂移、缺字段或
   无效值必须失败并要求重跑，不得输出貌似 current 的部分 evidence。
