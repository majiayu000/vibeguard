...
4. B-004 GitHub PR evidence 收集必须只读，并在读取 review threads 前后两次确认
   PR head 与 issue relation 未漂移；漂移、缺字段或无效值必须失败并要求重跑，
   不得输出貌似 current 的部分 evidence。
5. B-005 Offline PR gate 只有在 PR 非 draft、head/linked issue/current gate query、
   CI、review、review threads、review source、merge state 和 merge record 等全部
   确定性证据满足时才可越过 `blocked`；缺少的唯一条件是有效人工 merge 授权时，
   判定必须是 `needs_human` 而不是 `allowed`。
