...
- [ ] `SP621-T1` Owner: `/root` — 在 `tests/setup/` 新增 `runtime_install_tests.sh` focused matrix，由 aggregate runner 增加一条 source；先加入 helper existence/source wiring、entrypoint/helper syntax、`<800` / `<400` 行边界、attestation failure、strict unsupported target/unresolved tag、non-strict version mismatch fallback、helper missing/broken fail-before-side-effect 与 U-29 entrypoint/helper 独立 mutations，并把 no-cargo-metadata 断言指向 canonical helper；不得修改 799 行的 install-flow 或 854 行的 self-application 文件。Depends on: Spec PR merged and implementation route allowed。Covers: B-003, B-004, B-006, B-007, B-008。Done when: 未实现分支因缺少 helper 或入口仍超限确定性失败；每个新 mutation 有具名 status/output/no-fallback/no-persistent-write/no-network-build 断言；原 cargo-metadata 禁止断言未弱化；aggregate runner 仍低于 800 行。Verify: `bash tests/test_setup.sh`（production extraction 前保留预期失败证据）。
...
- [ ] `SP621-T3` Owner: `/root` — 让 release contract 与 U-29 checker 扫描 entrypoint + helper 的完整安装表面，并使 T1 的两个独立 fake-repo mutations 通过。Depends on: SP621-T2。Covers: B-003, B-004, B-007。Done when: 原安全文案断言继续通过，bad entrypoint 与 bad helper mutations 分别使 U-29 检查失败，不添加复制安全字符串的注释，`tests/test_self_application_ci.sh` 保持未修改。Verify: `bash tests/test_release_workflow.sh`; `bash tests/test_setup.sh`; `bash tests/test_self_application_ci.sh`。
...
- `stop_conditions`: 任一 runtime 函数体/错误语义需要非机械修改；公共 CLI 或安装顺序改变；
  必须弱化/删除现有断言；helper source-time 产生副作用；aggregate/setup flow/self-application
  任一文件被本 PR 净增长到/继续高于 800 行；任一 focused/full/quick gate 失败；
...
- `lane_map`: implementation `/root` 独占 `scripts/setup/install.sh`, `scripts/setup/` 下计划新增的
  `runtime-install.sh`, `tests/test_setup.sh`, `tests/setup/` 下计划新增的
  `runtime_install_tests.sh`, `tests/test_release_workflow.sh`,
  `scripts/ci/self-application/check-u29-no-silent-degrade.sh`；`tests/setup/install_flow_tests.sh`
  与 `tests/test_self_application_ci.sh` 明确只读；independent reviewer `/root/review_pr612` 只读，无可写文件。
