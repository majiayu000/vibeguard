...
| Bash / Write classification | `vibeguard-runtime/src/hook_checks/bash.rs:148`; `vibeguard-runtime/src/hook_checks/mod.rs:86` | Bash 与 Write malformed path 共用 diagnostic；Write 在 `vibeguard-runtime/src/hook_checks/mod.rs:118` 把 U-16 baseline read failure 也返回 `Malformed` | 需要保留 fail-closed，并把 baseline failure 从 protocol input failure 分离 |
| Event persistence | `vibeguard-runtime/src/hook_orchestrator/pre_bash.rs:16`; `vibeguard-runtime/src/hook_orchestrator/mod.rs:198`; `vibeguard-runtime/src/hook_orchestrator/mod.rs:596` | diagnostic detail 经 `append_hook_event` 写 project log，并在 `vibeguard-runtime/src/hook_orchestrator/mod.rs:677` 复制到 global log | privacy 断言必须同时覆盖两个持久化目的地 |
