...
- [ ] `SP628-T3` 为 doc-path allowlist 增加 strict structured parser 与 usage/freshness/runtime-alias gate。Covers: B-004, B-005, B-006. Owner: implementation agent. Dependencies: spec approval. Done when: invalid format/category/source/scope、unused、duplicate、多重命中与 stale runtime alias fixtures 均失败，current canonical alias 通过；command-path validator 保持独立语义。Verify: doc-path focused fixtures 与既有 command-path regressions。
