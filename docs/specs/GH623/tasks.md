...
- [ ] `SP623-T2` Owner: `/root` — 逐段机械提取 wrapper、package-correction、policy、SEC-14 四个 focused fragments，并让 aggregate 在原位置按原顺序显式 source；共享 helpers、temp root、trap、preflight 与 summary 留在 aggregate。Depends on: SP623-T1。Covers: B-001, B-002, B-003, B-004, B-008。Done when: aggregate/children 均 `<400`；四个原 section block 文本与 base snapshot 一致；child 无重复 harness/trap；source path/order 确定；隔离 repo copy 中 missing child 与 syntax-broken child 都使真实 aggregate 非零且 EXIT cleanup 后隔离 `TMPDIR` 为空。Verify: shell syntax、line counts、base-vs-split block comparison、headings/assertion descriptions 的顺序敏感完整清单 diff，以及两个临时副本负路径命令。
