...
`claude-sonnet-5`、`claude-opus-4-8` 与 Haiku 4.5 ID。实现时记录核实日期 2026-07-17。

离线 validator 检查 schema、alias 闭集、default membership、URL、日期格式和 age window。
推荐 90 天 review window；若维护者选择其他窗口，必须在 spec review 记录原因。validator
只要求人工重新核实并更新 commit，不抓取网页、不调用 Models API。

dry-run/help 输出 requested alias、resolved ID、verified date/source；真实 run artifact 沿用现有
model 字段，若当前 schema 未区分 requested/resolved，可只新增向后兼容 optional evidence，
不得破坏历史 artifact reader。
...
| B-002 | manifest default + CLI parser | no-arg/dry-run resolves haiku；help snapshot |
| B-003 | passthrough resolver + artifacts | arbitrary full ID remains unchanged and is recorded |
| B-004 | offline freshness validator | missing/invalid/stale evidence fixtures return nonzero |
...
| B-007 | dry-run/help renderer | output contains requested/resolved/evidence fields |
...
- Compatibility: alias 语义升级，完整 ID passthrough 保留复现路径。
- Performance: 读取一个小 JSON/YAML 文件，非 hook hot path。
- Maintenance: time-based gate 会主动要求复核，窗口需避免无意义频繁红灯。

## 测试计划

- [ ] Unit: manifest validation、alias/default/passthrough、stale dates。
...
可回滚 manifest/resolver 与 alias ID。若新模型暂不可用，应要求用户显式 full ID 或经 review
调整 baseline；不得把 API failure 静默降级到旧模型。
