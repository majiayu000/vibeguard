...
1. B-001 repo alias 闭集必须解析到 baseline manifest 声明的第一方 Claude API ID：
   `haiku` -> `claude-haiku-4-5-20251001`、`sonnet` -> `claude-sonnet-5`、
   `opus` -> `claude-opus-4-8`。这些值是本规格在核实日选择的固定基线，不是运行时
   evergreen lookup。
2. B-002 `haiku` 继续是 manifest default alias 与默认 CLI 选择，并解析到 B-001 的 dated
   Haiku ID；升级高阶 alias 不得把无参数 eval 成本切换到 Sonnet/Opus。
3. B-003 用户传入不属于便捷名闭集的完整 model ID 时必须原样传给 API，并在 run artifact
   中记录 resolved ID，支持历史复现。
4. B-004 baseline 必须记录官方来源、UTC `verified_at` date 与固定 90 日 freshness window；
   证据缺失、日期非法、核实日期晚于 UTC 当前日期、或 age 超过 90 个完整日时，离线
   contract check 必须失败并要求人工复核。age 0..90（含第 90 日）有效，第 91 日失效。
...
7. B-007 dry-run 必须显示本次 requested input、resolved ID 与 baseline evidence；help 必须
   显示 default alias、完整 alias -> resolved ID 表与 evidence，使未发 API 请求的预检也能
   发现运行目标。

## 验收标准

- [ ] `sonnet`/`opus` 解析到官方当前 ID，默认仍为 `haiku`。
- [ ] `haiku` 精确解析到 `claude-haiku-4-5-20251001`，不使用会移动的短 alias。
- [ ] 完整历史 ID 原样透传并记录。
- [ ] missing/invalid/future/day-91 baseline evidence 离线失败，day-90 仍有效且无网络调用。
