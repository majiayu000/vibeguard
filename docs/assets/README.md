...
| `demo-scenario.sh` | Scripted scenario that invokes real VibeGuard guards and the live safe-bash interception demo |
...
- Its destructive-command scene calls `setup.sh demo safe-bash`, which submits a real `PreToolUse(Bash)` request to `pre-bash-guard`; it does not print a fabricated decision.
