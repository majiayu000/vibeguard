...
pub(crate) fn run(args: &[String]) -> Result {
    ...
    if kind == HookKind::PreBash {
        ...
        crate::hook_orchestrator::pre_bash::run(&ctx, &input, start)?;
        return Ok(());
    }
    if kind == HookKind::PreEdit {
        ...
        crate::hook_orchestrator::pre_edit::run(&ctx, &input, start)?;
        return Ok(());
    }
    if kind == HookKind::PostWrite {
        ...
        crate::hook_orchestrator::post_write::run(&ctx, &input, start)?;
        return Ok(());
    }
    if kind == HookKind::PostEdit {
        ...
        crate::hook_orchestrator::post_edit::run(&ctx, &input, start)?;
        return Ok(());
    }
    if kind == HookKind::Stop {
        crate::hook_orchestrator::stop::run(&input, start)?;
        return Ok(());
    }
    if kind == HookKind::Learn {
        crate::hook_orchestrator::learn::run(&input, start)?;
        return Ok(());
    }
