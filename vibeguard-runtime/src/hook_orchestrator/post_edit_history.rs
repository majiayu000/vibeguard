...
use crate::hook_checks::common::{first_detail_path, known_w14_session};
use crate::hook_checks::history::{read_tail_lines, recent_overlap};
...
pub(crate) fn detect_history_warnings(
    ...
    // silent (issue #691).
    let session_temp = crate::hook_checks::common::is_session_temp_path(file_path);
    detect_churn(
        ctx,
        start,
        file_path,
...
#[cfg(test)]
#[path = "post_edit_history_unit_tests.rs"]
mod tests;

#[cfg(test)]
#[path = "post_edit_history_tests.rs"]
mod review_tests;
