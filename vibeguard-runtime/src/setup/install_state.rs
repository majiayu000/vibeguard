use crate::setup::quarantine_inventory::carry_incomplete_inventory;
use crate::setup::support::{SetupResult, home_dir, sha256_file, write_json_atomic};
...
pub(crate) fn validate_state_for_preflight_with_released_inventory(
    ...
    crate::setup::managed_tree_remove::validate_state_metadata(state)?;
    crate::setup::managed_tree_remove::validate_state_artifacts_with_released_inventory(
        state,
        released_inventory,
    )?;
    let version = state
