...
#[test]
fn codex_visible_failure_helper_maps_events() {
    let pretool = run_runtime_with_stdin(&["codex-visible-failure", "PreToolUse"], "blocked");
    assert_eq!(pretool.status.code(), Some(0));
    let pretool_json: serde_json::Value = serde_json::from_slice(&pretool.stdout).unwrap();
    assert_eq!(
        pretool_json["hookSpecificOutput"]["permissionDecision"],
    ...
    let posttool = run_runtime_with_stdin(&["codex-visible-failure", "PostToolUse"], "blocked");
...
#[test]
fn codex_diag_helpers_append_jsonl() {
    let root = unique_temp_dir("codex-diag");
    fs::create_dir_all(&root).unwrap();
    let diag_file = root.join("codex-wrapper.jsonl");

    let diag = bin()
        .args([
            "codex-diag",
            diag_file.to_string_lossy().as_ref(),
        ...
        .output()
        .unwrap();
    assert_eq!(diag.status.code(), Some(0));
    ...
    let lines = fs::read_to_string(&diag_file).unwrap();
    assert!(
        lines.contains("\"reason\":\"normalizer-failed\""),
        "{lines}"
    );
    assert!(lines.contains("\"hook\":\"pre-bash-guard\""), "{lines}");
    assert!(lines.contains("\"timeout_ms\":10000"), "{lines}");
