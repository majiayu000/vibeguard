...
- Codex hook installation follows the selected setup profile: every profile covers native Bash and `apply_patch` gates; `full` and `strict` additionally install `Stop` hooks (`stop-guard` and `learn-evaluator`). Read, Glob, and Grep hook surfaces are unavailable, so do not claim those loops were mechanically intercepted.
