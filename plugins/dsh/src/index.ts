...
import { HOOK_IDS, type GuardDecision, type PluginConfig } from './types.js'
...
function context(decision: GuardDecision, summary: string): UserMessage {
...
function prepend(ours: UserMessage, theirs: UserMessage[] | undefined): UserMessage[] {
  return [ours, ...theirs ?? []]
}

function workdir(agent: Agent | undefined): string {
...
export function apply(ctx: Context, config: Config): void {
  ...
  const tracker = new VerificationTracker(
    mutatingTools,
    compilePatterns(config.verificationCommandPatterns ?? DEFAULT_VERIFICATION_PATTERNS),
  )
  ctx.effect(() => async () => gate.dispose(), 'vibeguard-dsh: drain SessionStart hook')

  ctx.on('agent/session-start', ({ agent, source }) => {
    if (!runner.has('count-active-constraints')) return
    gate.start(agent, signal => runner.run(
      'count-active-constraints', sessionPayload(agent, source), workdir(agent), signal,
    ))
  })

  ctx.on('agent/pre-step', async ({ agent }, next): Promise<PreStepDecision> => {
    const startup = await gate.take(agent)
    if (startup?.decision === 'deny') return { kind: 'reject' }
    const downstream = await next()
    if (startup === undefined || startup.signals.length === 0 || downstream.kind !== 'enter') return downstream
  ...
  ctx.on('tools/pre-execute', async (exec, next): Promise<PreToolDecision> => {
    ...
    if (plan.preHooks.length === 0) return next()
    const decision = await runner.runMany(
      plan.preHooks, toolPayload(exec, plan, 'PreToolUse'), workdir(exec.agent), exec.signal,
    )
    if (decision.decision === 'deny') return { kind: 'deny', reason: serializeDecision(decision) }
  ...
  ctx.on('tools/post-execute', async (exec, result, next): Promise<PostToolDecision> => {
    const plan = toolHookPlan(exec)
    if (plan.postHooks.length === 0) return next()
    const decision = await runner.runMany(
      plan.postHooks, toolPayload(exec, plan, 'PostToolUse', result), workdir(exec.agent), exec.signal,
    )
    if (decision.decision === 'deny') {
      return { kind: 'block', feedback: [{ type: 'text', text: serializeDecision(decision) }] }
    }
    const downstream = await next()
    if (decision.signals.length === 0) return downstream
  ...
  ctx.on('agent/turn-stopping', async ({ agent, turn, signal }) => {
    ...
    if (stops.first(agent, turn)) {
      stopDecision = await runner.runMany(
        ['stop-guard', 'learn-evaluator'], stopPayload(agent, turn, false), workdir(agent), signal,
      )
    }
