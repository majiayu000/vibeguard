...
import { HOOK_IDS, type GuardDecision, type HookId, type PluginConfig } from './types.js'

const SCRIPT_NAMES: Record<HookId, string> = {
...
export class HookRunner {
  ...
  private readonly enabled: ReadonlySet<HookId>
  ...
  constructor(
    ...
    for (const hook of this.enabled) {
      ...
      if (!existsSync(path)) throw new Error(`vibeguard-dsh: hook not found at ${path}`)
    }
  }
  ...
  private path(hook: HookId): string {
  ...
  async run(
    ...
    if (!this.has(hook)) return mergeDecisions([])
  ...
  async runMany(
    ...
    for (const hook of hooks) {
      if (this.has(hook)) decisions.push(await this.run(hook, payload, workdir, signal))
    }
